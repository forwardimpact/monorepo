...
# worktree creation. It does not run after that default.
#
...
#            versions of this script read `.new_branch`, which never exists.
#            So the hook always exited 1. It never created a worktree.
...
#            diagnostic goes to stderr. (A stray stdout line caused an earlier
#            bug. The harness misread bootstrap.sh's "pull: up to date" as the
...
#            worktree before we fail.

# ── Inputs ───────────────────────────────────────────────────────
payload=$(cat)
new_branch=$(jq -r '.name // empty' <<<"$payload")
# The payload supplies no base ref. Branch from main to match the repo's
...
# own resume behaviour: if git already registered a worktree at this path, hand
# it back untouched. Do not fail on "already exists".
...
# Set the trap before `worktree add`. The trap then always tears down a
# half-built worktree (add succeeds but a later step dies).
...
if git -C "$repo_root" show-ref --verify --quiet "refs/heads/$new_branch"; then
  # The branch already exists. Someone removed its worktree and kept the
  # branch. Attach a worktree to it. Do not create the branch again.
