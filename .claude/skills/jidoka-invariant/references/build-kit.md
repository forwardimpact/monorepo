...
- `match(name)` — a filename predicate that selects which files become
  subjects.
- `skip` — directory *names* the walk prunes (`["node_modules", "dist",
  "test"]`). They are names. They are not globs.
- `under: "src"` — restrict each `dirs` entry to the per-package
  `<dir>/<child>/src/**` shape. Use this to scan `src` and to skip its `test`
  sibling.
...
- A file that fails to parse becomes `{ path, rel, parseError }` instead. Always
  guard that scope with the `parseError` rule. A syntax error then surfaces as a
  finding, and the engine does not drop the file silently.
...
depth-first over every typed node, and it skips `loc`/`start`/`end`. Accumulate
into a closure:
...
- `grep` is **case-insensitive unless you set `caseSensitive`**.
...
- `exclude` — a RegExp that `grep` tests against the raw `rel:lineNo:text`
  line, to drop false positives.
...
- It matches each consumer line by line, and emits one subject per match:
  `{ key, path, lineNo, restated, expected, ok }`. The match is native and does
  not use ripgrep, so colons in URLs survive.
...
To add a topic of an existing source kind, edit one registry line. Write no
code.

## Read, config, and list helpers
...
- `config(name, fallback?)` → JSON/YAML the kit reads **beside the module**. It
  resolves the name against `dir` and parses by extension. Returns `fallback`
  (default `null`) when absent, empty, or unparseable. Keep deny/allow lists
  here so the rule and its data version together.
...
- `glob(pattern)` → an anchored `RegExp` that matches `rel` (`**` spans
