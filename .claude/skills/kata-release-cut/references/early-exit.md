...
This file holds the normative rules for SKILL.md § Step 2. A `NO-CUT-OWED`
verdict requires all four conditions below. When any condition fails, or any
check is in doubt, the verdict is `SWEEP-REQUIRED`. Worked invocations and
traversal hazards live in [early-exit-mechanics.md](early-exit-mechanics.md).
...
- Every classification binds a SHA pair. `range_from` is the baseline `B`.
  ...
  pair. It is never a claim about live `HEAD`.
- The intended failure mode is forgone savings. A missed cut is never the
  intended failure mode. Doubt always classifies toward the sweep.

## Condition 1 — Verified-clean baseline

Condition 1 needs a commit `B`. A prior run record cites `B`, and `B` is an
ancestor of `HEAD`. At `B` an assessment verified zero unreleased commits
beyond what it re-cited as blocked. A valid `B` comes from a full sweep that
reaches that state, from a post-cut state, or from a chained earlier
early-exit. On a shallow checkout `B` can sit below the fetch boundary. Deepen
the clone until the ancestry check can run. If you still cannot reach `B`, the
baseline is unresolvable and the run sweeps.

## Condition 2 — Zero publishable paths over `B..HEAD`

Test the union of paths changed by each commit in the range. Never test a net
diff. Use two tiers at the frozen `range_to`:

1. **Directory tier.** A path under no publishable-package directory (from
   the workspace manifest at `range_to`) never defeats the exit.
...
`.gitignore` at any level in the package directory). The packlist refinement
excludes a package that declares `prepack`, `prepare`, or `prepublishOnly`.
All its paths stay publishable, because a build step is a genuine missed-cut
channel.
