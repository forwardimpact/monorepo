...
The standard governs head moves. A PR whose head did not move since its
approval signal stays covered by that signal directly. It needs no transfer
...
the plan PR's content. It does not certify the implementation head.
Implementation PRs stay outside this standard.
...
also certifies a commit pair. It does not certify a head state. A rebase onto a
main that itself changed the same paths legitimately fails this check, because
changes that overlap get fresh review by design.
...
the smuggled-commit gap. Nobody added commits alongside. The ancestry closes
the foreign-base gap. No foreign history sits beneath the PR. Together they
certify the PR is exactly its N commits that sit directly atop genuine main
history.

Establish the chain from the contemporaneous transfer records on the PR. Do not
establish it from git reachability. Force-pushed intermediate heads may be
unreachable later.
...
per the trust rule. `staff-engineer` can approve plans, so a voided plan signal
routes there. It does not route to human escalation.
