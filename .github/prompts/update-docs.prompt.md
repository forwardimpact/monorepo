...
2. **Audit against the current codebase**
   ...
   - Check `apps/schema/` for data structures and validation
   - Check `apps/model/lib/` for derivation logic
...
3. **Verify diagrams**
   ...
   - Compare derivation flowcharts against code in `apps/model/lib/`
...
| Job Derivation    | `apps/model/lib/job.js`              |
| Agent Derivation  | `apps/model/lib/agent.js`            |
