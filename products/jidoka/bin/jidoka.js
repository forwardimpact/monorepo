...
const definition = {
  name: "jidoka",
  description:
    "Enforce the layered instruction architecture defined in JIDOKA.md (no subcommand: run every check)",
  commands: [
    {
      name: "instructions",
      args: [],
    ...
    },
    {
      ...
      examples: ["jidoka invariants", "jidoka invariants --seed ambient-deps"],
    },
    {
      name: "jtbd",
      args: [],
      ...
      handler: jtbdHandler,
      examples: ["jidoka jtbd", "jidoka jtbd --fix"],
    },
  ],
  globalOptions: {
  ...
  examples: ["jidoka", "jidoka instructions", "jidoka jtbd --fix"],
  documentation: [
    {
      title: "Jidoka Instruction Architecture Standard",
      url: "https://github.com/forwardimpact/monorepo/blob/main/JIDOKA.md",
    },
    {
      title: "Jidoka website",
      url: "https://www.jidoka.team/",
    },
  ],
};
...
async function runInstructions(root, jsonOutput, rt) {
  ...
  writeFindings(findings, "jidoka instructions passed", jsonOutput, root, rt);
...
async function runJtbd(root, fix, jsonOutput, rt) {
  ...
  writeFindings(findings, "jidoka jtbd passed", jsonOutput, root, rt);
...
async function invariantsHandler(ctx) {
  ...
  const findings = await runRuleModules(modules, {
    root,
    runtime: rt,
    dir: resolve(root, INVARIANTS_DIR),
  });
  writeFindings(
    findings,
    "jidoka invariants passed",
    !!ctx.options.json,
    root,
    rt,
  );
