...
describe("agent-builder-install", () => {
  describe("getPackName", () => {
    test("uses discipline abbreviation + kebab-case track id", () => {
      const discipline = { id: "software-engineering" };
      const track = { id: "platform" };
      // "se-platform" matches the name that build-packs.js generates
      assert.strictEqual(getPackName(discipline, track), "se-platform");
    });

    test("uses the kebab-case track id verbatim", () => {
      const discipline = { id: "data-engineering" };
      const track = { id: "infra-tools" };
  ...
  describe("getRawCommand", () => {
    test("pipes the download through tar to extract directly", () => {
      assert.strictEqual(
  ...
  describe("createInstallSection", () => {
    // createInstallSection returns null for a falsy siteUrl before it
    // touches any DOM API. So this branch is safe to exercise in node.
    // The happy path renders with `document.createElement`. The Playwright
    // suite covers that end-to-end. This file does not duplicate it.
    test("returns null when siteUrl is undefined", () => {
      const result = createInstallSection({
        discipline: { id: "software-engineering" },
        track: { id: "platform" },
        siteUrl: undefined,
      });
  ...
  // archive filename that build-packs.js emits. If either side changes how
  // it builds the name, this test fails before users copy a broken install
  // command.
    ...
    test("every valid combination's getPackName matches an emitted archive", async () => {
      ...
      for (const { humanDiscipline, humanTrack } of combinations) {
        ...
        assert.ok(
          ...
          `expected packs/raw/${expected} to exist. The UI pack-name formula drifted from build-packs.js`,
        );
      }
    });
