...
| `artifact-name`    | No       | `benchmark-results`| Name for the uploaded artifact. Run mode with `shard-total` > 1 uploads `benchmark-shard-<i>`. The name must not contain `--`, the trace-name delimiter. The action fails fast otherwise |
