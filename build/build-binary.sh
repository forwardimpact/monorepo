...
#
# This script resolves <NAME> to the package.json that declares bin[<NAME>].
# It then compiles the entry with two build-time literals that libcli consumes:
#
#   LIBCLI_PACKAGE_VERSION  the built package's version. `bun --compile` mounts
#                      source onto a virtual /$bunfs filesystem, so
#                      readFileSync(package.json) ENOENTs at runtime.
#                      resolveVersion reads this literal instead, and the
#                      readFileSync fallback tree-shakes away.
...
#
# `--define` substitutes each literal `process.env.<NAME>` token across the
# whole bundle (the bundled libcli included). The build compiles each binary
# separately, so the shared names carry that binary's own values.
#
# build/gen-embed.mjs inlines the package-data assets the CLI declares in
# build/cli-manifest.json (prompts, templates). It prints the entry to compile.
# That entry is a generated shim that registers the assets first. When the CLI
# declares none, it is the original entry unchanged.
set -euo pipefail
