...
# It is the Linux analog of the macOS cask. The tap ships a committed cask
# skeleton, whose binary block render-cask-binaries.sh regenerates. But the tap
# ships NO formula skeleton, so this script writes the whole file. That also
# means a new bundle folds in with no hand-seeded tap file. Its tag plus a
# manifest entry is enough.
#
...
#
# The formula shape fills the version and both per-arch sha256 directly, with
# no second rewrite pass. The tarball holds only self-contained CLI
# executables, so `bin.install Dir["*"]` installs exactly the bundle's manifest
# CLIs. The script ends with `ruby -c` as a syntax gate.
set -euo pipefail
...
# Homebrew derives the class name from the file token. It capitalizes each
# "-" segment and joins them (fit-gear -> FitGear, fit-outpost -> FitOutpost).
