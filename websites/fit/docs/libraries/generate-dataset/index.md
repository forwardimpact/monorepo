...
Read [The Substrate Contract](/docs/libraries/substrate-contract/) next. The
contract explains how to provision identities and issue credentials against
...
[The Substrate Contract](/docs/libraries/substrate-contract/) shows how the
generated standard reaches a running stack. A complete `standard` block declares
capabilities, behaviours, disciplines, and levels. The `seed` field makes the
entity graph deterministic. The same seed produces the same people, assignments,
and proficiency ratings on every run.
