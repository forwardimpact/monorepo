...
already exists. Before an engineer can read their own activity rows, their
roster entry needs a paired `auth.users` row.
...
The verb reads the roster through the
...
- `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` available in your
  ...
  reads. The verb lives on `fit-terrain` and not on `fit-landmark`.
  Landmark's read path never holds the service-role key.
- Your stack implements and populates `substrate.people`. For Map
  installations, run
...
| `decommissioned` | The roster row no longer exists. `provision` banned the paired `auth.users` row (`banned_until` ≥100 years out). |
...
on this contract. Production operators can run the verb from cron. It does
not churn state.

## Decommissioning

When an engineer leaves and you remove their roster row, the next
`provision` run bans their `auth.users` row. It sets `banned_until` about
100 years out with `ban_duration: "876000h"`. A banned user cannot issue
a JWT. No new Landmark reads land for them.
...
same row with `ban_duration: "none"`. The unban preserves any audit trail
...
- **Issue JWTs to engineers.** `provision` only makes sure the `auth.users`
  ...
  get a JWT when they run `fit-landmark login` themselves, through the
  magic-link or OTP flow. Operators who issue long-lived tokens for
  unattended agents use `fit-map auth issue`. See
