...
The action exposes most `gemba-benchmark run` CLI flags as action inputs.
`--task`, `--skills-from` and `--work-tracker` have no input, and `--shard=i/N`
splits into `shard-index` and `shard-total`. The action also adds CI-specific
inputs that have no CLI equivalent:
...
The two family names come from the platform's reference tenant,
[Kata](https://www.kata.team/), which grades two skill packs in one workflow.
Use your own family paths and names.
