...
# Exit early after a gate fails. The gate row already carries the failure.
fit-trace assert file-present --gate --exists "$DESIGN" >&"$RESULTS_FD" || exit 0

# Design § over-200-lines is a Blocker (kata-review delta). It is a gate.
LINES=$(wc -l < "$DESIGN")
...
# common way an author names a rejected option. Case-insensitive (RegExp /im/).
