// The kata-, coaligned-, and monorepo-pack skills follow the house style
...
// three packs, plus the bare-CLI rule for the coaligned-* and monorepo-*
// packs genericity does not scan. fit-* skills are out of scope for both —
// they have not been converged on the template.
//
// One deliberate exclusion: monorepo-setup's check-workflows.md CI templates
// keep `npx coaligned` — those generated workflows run `npm ci` on clean
// runners with nothing on PATH, so the registry-resolvable form is the rule
// there (coaligned-setup Step 4).

const PACK_SKILL =
  /^\.claude\/skills\/(kata|coaligned|monorepo)-[^/]+\/SKILL\.md$/;
...
export default {
  ...
  rules: ({ failAll }) => [
    ...
    failAll("style-match", {
      ...
      hint: "published skills share one voice (.claude/skills/CLAUDE.md § House style); narrow the rule in .coaligned/invariants/skill-template.rules.mjs only for a legitimate usage",
    }),
  ],
};
