...
| C7 | Bundled runtime: set `outputFileTracingRoot` for the standalone build; set `POLARIS_ABOUT_PATH` for the bundled YAML read; use plain `bun install`; serialize the apm pack reconstitution (the apm resolver races creating its `~/.apm` directory under parallel resolution). Evidence: `612f950`, `eb1114a` | A flat `server.js` the Dockerfile cannot copy, an unreadable about file, a rejected install flag, a bootstrap that fails intermittently on the pack download race |
