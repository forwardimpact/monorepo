...
Wire Railway watch-path deployments for each infrastructure service and
product surface. Add one end-to-end smoke script. The script verifies every
spec-1160 success criterion against a newly booted stack.

All paths are inside `bionova-apps/`.

## Step 1 — Create Railway project

Created (with the Railway CLI, not committed):
...
Document the Railway project id in `.railway/project.json`. The CLI creates
that file. Add the file to gitignore. Document the project name in
`infrastructure/railway/README.md`.

If you have no Railway account access, stop this step. Document the gap in
the part-08 PR description. The later steps stay valid for local-only
verification.
...
Verify: `railway up --service postgres` deploys. A later push that changes
only `products/polaris/site/` redeploys `polaris-site`. It does not redeploy
postgres.
...
`products/polaris/site/src/app/page.tsx` triggers a deploy of `polaris-site`
only.
...
Also created: `scripts/fixtures/eligible-patient.json`. The file holds one
object with `trial_id` and `payload`. `trial_id` holds the id of one specific
seeded trial. `payload` holds an eligibility request for a patient who
matches. The payload satisfies every inclusion criterion of that trial.
`scripts/build-fixture.sh` (below) regenerates the fixture. That script
queries the live DB, so the fixture stays in sync with the vendored seed. The
committed JSON is the output of that script against the rendered seed. That
seed comes from the DSL vendored at the provenance SHA in
`data/synthetic/PROVENANCE.md`.
...
on the live DB, so run it after `setup.sh`. When the seed schema or the
criterion shape changes (i.e., spec 1150 evolves), run
`./scripts/build-fixture.sh` again. Commit the regenerated JSON in the same
PR. You need no separate README. The script is the canonical procedure.
...
Fill in `.github/workflows/check-e2e.yml`. Part 01 scaffolds the file.
SHA-pin both actions per part 01 step 9. The file is a standalone
per-concern workflow with no `needs:` on other jobs. The other concerns run
as their own workflows:
...
The `.env.example` placeholder values are enough. The anon key and the
service-role key apply only to the ephemeral CI Postgres instance. The
...
| `docs/operations.md` | Day-2 operations: re-seed, scale TEI, rotate the service-role key |

Verify: the docs link from the root README. `markdownlint docs/` passes.
...
Verify: the README renders cleanly on GitHub. The quickstart instructions
match what `scripts/smoke.sh` exercises.
...
Verify: the PR CI is green (all jobs). The e2e job reports that 7 SCs pass.
...
it only after the part-08 PR in `bionova-apps` merges AND the smoke
script passes against the merged `main` of `bionova-apps`. The PR flips
the monorepo's `wiki/STATUS.md` row. `kata-release-merge` and the
storyboard then see the lifecycle close.
...
`1160<TAB>plan<TAB>implemented`. Use literal tabs. Do not use the `\t` escape.
Verify locally with `grep -P '^1160\tplan\timplemented' wiki/STATUS.md`. The
`-P` flag reads `\t` as a tab. If grep returns nothing, the row uses spaces
instead of tabs, and `kata-release-merge` will not flip the spec. Edit the row
again with an editor that keeps tabs. Run the grep again.

Append to the `wiki/staff-engineer-2026-W<NN>.md` log for the current ISO
week. Use `date -u +%G-W%V` to resolve the week. Record that the
implementation of spec 1160 completed. Record the bionova-apps repo URL, the
merged-PR list, and the smoke result. Append a metrics row per
...
- [ ] `scripts/smoke.sh` exits 0 against a fresh stack: SC1–SC7 all pass.
- [ ] The CI `e2e` job runs end-to-end on every PR. A failure uploads
      `docker compose logs`.
- [ ] The Railway project deploys each service on a watch-path change.
- [ ] `wiki/STATUS.md` shows `1160    plan    implemented` after the trailing
      monorepo PR merges.
- [ ] `https://github.com/forwardimpact/bionova-apps` is public and builds
      green. The README quickstart works against a fresh clone.
