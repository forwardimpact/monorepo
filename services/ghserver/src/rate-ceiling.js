 ...
 *
 * The class keeps a sliding 60-second window of mint timestamps per
 * `tenant_id` in an in-process map. A rate ceiling does not need
 * durability across restarts. A restart resets the window. That fails
 * open for at most one window. It never blocks a legitimate caller.
 * Inject `{ clock }` so tests drive the window with a virtual clock
...
export class RateCeiling {
  ...
  /**
   * @param {string} tenant_id
   * @returns {boolean} True if `tenant_id` reached the ceiling in the
  ...
  /**
   * Drop timestamps older than 60 seconds. Return the live window
