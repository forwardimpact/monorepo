...
How to configure the **GitHub user App**. This is the Kata Agent **User** App
that issues per-user OAuth (user-to-server) tokens through `services/ghuser`.
Both bridges share it. It supplies the GitHub identity a `workflow_dispatch`
runs under in **both** deployment models. The human who asked therefore authors
the commits. A shared bot does not.

> **Two GitHub apps, do not conflate them.** This guide covers the **user App**
> (per-user OAuth). The separate **server App** is the installation App whose
> key mints server-to-server tokens. See
...
| Per-user linking | users link once through the OAuth flow. The bridge prompts on the channel when a link is missing | the same flow. Every hosted dispatcher links once before dispatch |
...
`${GHUSER_LINK_BASE_URL}/callback`. Run `services/ghuser`.
...
(`ghbridge` or `msbridge`) prompts on the channel when a link is missing. The
bridge resolves the per-user token through `ghuser` at dispatch time.

## Hosted (multi-tenant)

Hosted `workflow_dispatch` uses the user App, exactly as self-hosted does.
`services/ghuser` resolves the dispatch credential to the per-user OAuth token
of the user who dispatches. The human dispatcher therefore authors the hosted
workflow commits (the per-user attribution the design's later trade-off
restored). The resolved tenant repo comes from `services/tenancy`. The dispatch
credential does not vary by mode.

Every hosted dispatcher links their GitHub account once before dispatch. This
uses the same OAuth flow and configuration as self-hosted above.
`services/ghuser` is therefore required on the dispatch path in both models.

## See also

- [GitHub **server** App configuration](https://github.com/forwardimpact/monorepo/blob/main/services/ghserver/github-app.md)
  — the other GitHub app (installation tokens), with the self-hosted and
  hosted scenarios.
- [Azure AD app configuration](https://github.com/forwardimpact/monorepo/blob/main/services/msbridge/azure-app.md)
  — the Teams app. It shares this user App for dispatch identity in both
  models.
