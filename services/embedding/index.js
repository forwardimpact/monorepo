...
/**
 * gRPC service that produces text embeddings. It delegates to a
 * Text Embeddings Inference (TEI) HTTP backend over its OpenAI-compatible
 * `/v1/embeddings` endpoint. `server.js` spawns the TEI process. This
 * class is a thin adapter. It translates the proto request/response shapes
 * to and from TEI's HTTP payload.
 *
 * Implements the `Embedding.CreateEmbeddings` RPC that
 * `proto/embedding.proto` defines. See
 * `generated/services/embedding/service.js` for the `EmbeddingBase` it
 * extends.
 */
export class EmbeddingService extends EmbeddingBase {
   ...
   *   The constructor throws if it is empty.
   */
  ...
  /**
   * Embed one or more input strings through the TEI backend.
   *
   * @param {{input: string[]}} req - Proto-decoded `EmbeddingsRequest`.
   *   `input` is the list of text strings to embed. The response keeps
   *   the order.
