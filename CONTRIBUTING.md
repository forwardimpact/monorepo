...
- [ ] **Search shared libraries first.** Before writing a helper, utility, retry
      ...
      `libraries/` and the relevant `libs-*` skill group for an existing
      capability. If a shared library already provides it, use it. If not, note
      that in the plan or commit so future contributors don't re-search.
