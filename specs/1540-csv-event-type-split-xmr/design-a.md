...
CSV under `wiki/metrics/**/2026.csv` *whose existing header matches the
pre-migration `EXPECTED_HEADER`* — every per-agent CSV *and* every
kata-skill CSV — is rewritten once during this PR series. Two files in
that glob already diverge from `EXPECTED_HEADER` on `main` and are
carved out by name; see § Outlier CSVs. Runtime code (recorder,
validator, analyzer, chart, summarize) assumes the new column
unconditionally; there is no schema-version branch, no header dispatch,
no v1/v2 coexistence. The migration is performed by a one-time script
that is removed from the codebase in the same PR series.
...
| `DEFAULT_SHIFT_TYPE` couples to a workflow filename's `name:` field | Accepted. The constant is a literal string in `constants.js`; if `.github/workflows/kata-shift.yml` ever renames its `name:`, the read default silently re-slices | Compute the default from a workflow file at build time; symlink the name through a registry | Workflow file renames are rare and a no-rows-after-filter result is highly visible at the next storyboard refresh. A registry adds a moving part for a problem with a loud failure mode. Mitigation: a doc-comment on `DEFAULT_SHIFT_TYPE` and a one-line CHANGELOG check item ("if you rename a workflow `name:`, search `libxmr` constants and rg `wiki/metrics/` for the old value"). |
| Open-set validator cannot catch typos at write time | Accepted. The validator only checks non-empty; a typo like `Kata: Disptach` lands without complaint | Closed enum; soft-warn against a known-set registry; require a per-workflow allowlist | The smart default removes the dominant typo path — when `$GITHUB_WORKFLOW` is the source, the value is correct by construction. The residual risk is on explicit `--event-type <name>` invocations, which are rare and post-hoc auditable. A closed enum re-introduces the closed-set drift the open-set decision was made to avoid. Mitigation: storyboard `analyze` lists distinct `event_type` values seen — a typo shows up as a one-row slice. |
