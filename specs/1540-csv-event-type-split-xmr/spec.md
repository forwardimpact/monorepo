...
| Exp SE 1411-A (Issue #1412) and any future PDSA experiment recorded against a per-agent CSV can read a verdict against a non-contaminated baseline. | After the migration, drive the staff-engineer chart restricted to `shift-work` on the typed CSV metric that proxies the trace-derived signal Exp SE 1411-A observes. The experiment's own predicates (P1 turn-index ordering of `Read(*.{js,ts,…})` events vs. first `Write(plan-a*.md)`; pre-draft Read counts) live in the trace, not as a typed CSV field — this spec does not add a `read_calls` metric. The `tool_calls_total` series restricted to `shift-work` (per-tool breakdown in the row's `note`) is the typed proxy the chart reads; observe its μ, UPL, and signal classification are computed from shift-work rows alone. The artefact stands independent of #1412's own verdict horizon. |
