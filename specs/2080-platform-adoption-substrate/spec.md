...
per team and platform. GetDX ingestion is adapted to resolve its team
references into the registry rather than originate teams. The `getdx_teams`
table and every `getdx_team_id` reference are removed.
...
| GetDX re-key | `activity.getdx_snapshot_team_scores`, `activity.getdx_snapshot_comments` | Replace the GetDX team column on both tables with a reference to `activity.teams`. The team-scores row identity and its ingestion upsert key move to (snapshot, team, item). The comments table keeps its existing engineer-attributed (email) scope. |
...
| Platform usage cube | New `activity.platform_usage` table | Team × Platform × Metric × Period facts: value, contributor count, period bounds, prior-period comparison, and a discriminator marking each metric as usage or outcome. References `activity.teams` and `activity.platforms`. No per-engineer column. |
| Clean-break removal | Schema migrations | Remove `activity.getdx_teams` and every `getdx_team_id` reference — including the SQL function `activity.snapshot_ids_for_person`, which joins on it. Rewrite the base activity-schema and Landmark RLS migrations to the new shape and extend the RLS migration's retention-metadata validator allowlist to the new tables. Fold in and remove the standalone GetDX-team-column migration. No compatibility shim, no data migration. |
...
| GetDX ingestion adaptation | Map GetDX transform path | The GetDX team transform stops writing `getdx_teams` and stops setting membership; it resolves each GetDX team to a registry team via the source reference. The snapshot-score and comment transforms write the resolved team identity. GetDX teams that do not resolve are counted and skipped, never auto-created. |
| Readers re-pointed | `activity.queries.snapshots`, the Map substrate persona and roster surfaces, and the Landmark `sources` command | Every read that referenced `getdx_teams` or `getdx_team_id` reads the registry and the new team identity instead. The `get_team` management-tree helper is orthogonal and unchanged. |
| Pull collector + adapter contract | New `fit-map platform ingest` verb and a platform-adapter interface | Generic collector dispatches to a registered adapter by platform identity and upserts the cube idempotently per period. Collector-enforced guarantees: cohorts below the contributor threshold (default 5) are dropped before insert; a team that does not resolve is counted and skipped; an adapter emitting any per-person field — in its rows or its raw passthrough — is rejected. A reference fixture adapter exercises the path. |
...
| Tests | `products/map/test/`, `products/landmark/test/` | Cover registry authorship idempotency; RLS subtree scope and anon-zero-rows on every team-attributed table; GetDX resolution and unresolved-skip; collector threshold drop, team-resolution skip, per-person rejection, and idempotency; adoption-matrix suppression, usage-needs-outcome rendering, and the correlation join; and that no `getdx_teams`/`getdx_team_id` reference survives anywhere. |
...
and the collector rejects any per-person field) and team-aggregate cohorts below
a contributor threshold are dropped (k-anonymity). The remaining constraints —
Claude Code content gates default-off, trace bodies engineer-only — bind the
follow-up adapter specs (Scope-out), which cite #829; they are not restated here
so the issue stays canonical and this slice ingests no vendor content to test
them against.
...
| # | Claim | Verification |
| --- | --- | --- |
| 1 | `activity.teams`, `activity.platforms`, and `activity.platform_usage` exist; `activity.getdx_teams` does not; no `getdx_team_id` column exists anywhere in the `activity` schema. | Static check on the migrated schema: the three tables are present, `getdx_teams` is absent, and no `getdx_team_id` column exists. |
...
| 3 | `fit-map teams push` populates `activity.teams` from an authoritative roster and is idempotent over an unchanged roster. | Behavioural test: push a roster of N teams; assert N registry rows; re-push; assert the row count and per-team identity are unchanged. |
...
| 5 | An authenticated caller reads team-attributed rows only for teams in their subtree; a sibling subtree contributes zero; an anon caller reads zero. | Behavioural test with a multi-level team fixture: a team lead sees own and descendant team rows but not a sibling subtree's; anon reads zero from each team-attributed table. |
| 6 | GetDX ingestion resolves each GetDX team to a registry team via the source reference and writes scores and comments against the resolved team; an unresolved GetDX team is counted and skipped, not created. | Behavioural test: seed the registry with a source reference for one GetDX team and omit another; run the GetDX transform; assert scores and comments for the mapped team carry its registry identity, the unmapped team produces no `teams` row, and the skip is counted. |
| 7 | The collector drops a usage row whose contributor count is below 5, skips a row whose team does not resolve, and rejects an adapter that emits a per-person field in its rows or raw passthrough. | Behavioural test via the fixture adapter: feed a sub-threshold row, an unresolved-team row, and a per-person-field row; assert none are stored, the drops and skips are counted, and the per-person case errors. |
| 8 | `fit-map platform ingest` is idempotent per period. | Behavioural test: ingest a fixture period twice; assert the cube row count and values are unchanged. |
| 9 | `fit-landmark adoption` renders Team × Platform for a period, suppresses sub-threshold cells on both the usage and GetDX sides, and renders a usage metric only beside its platform's outcome metric. | Behavioural test: seed usage rows, some sub-threshold and one usage metric with no paired outcome; assert sub-threshold cells are absent or rolled up and the unpaired usage cell is withheld behind the empty-state registry key. |
| 10 | `fit-landmark adoption --platform <id>` draws a per-team, per-platform control-chart trend. | Integration test: seed enough periods; assert the chart renders with control limits via the existing XmR machinery. |
| 11 | The adoption surface correlates a team's platform usage with its GetDX scores by mapping a usage period to the GetDX snapshot cycle whose schedule covers it, joined on the shared team identity. | Integration test: seed `platform_usage` and `getdx_snapshot_team_scores` for the same team in a usage period that falls within a snapshot cycle; assert the correlation view returns paired usage and score values. |
| 12 | The operator write credential for `fit-map teams push` and `fit-map platform ingest` is not reachable from any Landmark read path. | Static inspection: no file under `products/landmark/src/` constructs a service-role client or references the write credential, mirroring spec 0840 criterion 3a. |
| 13 | No source, test, or migration references `getdx_teams` or `getdx_team_id` after the change. | Static inspection: a search for `getdx_teams` and `getdx_team_id` across `products/map/` and `products/landmark/` (source, tests, and migrations) returns zero occurrences. |
...
- Team-scope RLS idiom to generalize, plus the retention validator and accessor
  functions to extend: the Landmark RLS migration
  (`20260510000000_landmark_rls.sql`). The `activity.snapshot_ids_for_person`
  function there joins on `getdx_team_id` and must be re-keyed.
- GetDX team origin to demote: the team, score, and comment transforms in the
  Map GetDX transform path (`products/map/src/activity/transform/getdx.js`).
- GetDX-team readers to re-point: the snapshots query
  (`products/map/src/activity/queries/snapshots.js`), the substrate persona and
  roster surfaces (`substrate-persona-query.js`, `substrate-roster.js`,
  `persona-enricher.js` — note the latter's id-format coupling into the
  synthetic DSL), and the Landmark `sources` command
  (`products/landmark/src/commands/sources.js`).
- Operator write-path and idempotency precedents: `fit-map people push`
  (`products/map/src/commands/people.js`) and `fit-map people provision`
  (`products/map/src/commands/people-provision.js`).
- Standalone GetDX-team-column migration to fold in and remove:
  `20250504000001_org_people_getdx_team_id.sql`. The comments FK lives in
  `20250101000003_getdx_snapshot_comments.sql`.
...
- Umbrella: [#829](https://github.com/forwardimpact/monorepo/issues/829),
  §"Goodhart risk" and §"Recommended posture".
