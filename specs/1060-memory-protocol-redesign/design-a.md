...
| **`fit-wiki refresh`** | Extended. Continues to regenerate `<!-- xmr:... -->` chart blocks; gains `<!-- issues:label=<l>:state=<s>[:closed_within=<N>d] -->` / `<!-- /issues -->` marker pairs that render the storyboard's Obstacle / Experiment lists from `gh issue list` output. Idempotent; date-window math (e.g., `closed_within=7d`) handled by the renderer, not the coach. Writes stay with `gh issue create/comment/close` — `refresh` is read-only against the GitHub state. |
...
| `refresh` | extended | Storyboard XmR chart refresh **+ Obstacle / Experiment list blocks rendered from `gh issue list` (marker-bounded, idempotent, 7-day concluded-window math)** | — (closes the storyboard two-artifact-sync class observed in W20/W21 traces: orphan `Exp N` lines, missed `(#NNN)` suffixes, stale Concluded partition) |
...
| Obstacle / Experiment surface (kata-session integration) | GH issues remain the authoritative coordination artifact for obstacles and experiments (`gh issue create/comment/close` per `kata-session/references/issue-lifecycle.md`). The storyboard's `### Active` / `### Concluded (last 7 days)` lists become a *rendered reflection* of GH state via new `<!-- issues:... -->` marker blocks driven by `fit-wiki refresh`. Memory (the storyboard markdown) becomes mechanical; coordination (the issues) stays where it is. | Rejected: `fit-wiki obstacle` / `fit-wiki experiment` write subcommands wrapping `gh issue`. · Adds a parallel write surface for one consumer (the coach), duplicates `gh`, and crosses the memory/coordination boundary that the rest of the redesign preserves. The failure mode in 5/17–5/18 storyboard traces (orphan `Staff Exp 41` with no labeled issue, +6d close-out lag on RE Exp 41, hand-edited `(#NNN)` suffixes, manual 7-day partition aging) is a *reflection-drift* failure, not a write-primitive failure — render-only closes it. |
