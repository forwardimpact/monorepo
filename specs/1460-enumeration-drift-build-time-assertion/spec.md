...
| The outcome-metric baseline is recorded at landing | The implementation PR's description records the spec id (1460), the merge SHA, and the merge date, so § Outcome metric evaluates against an explicit baseline rather than an inferred one |
| Implementation PR diff is scoped by the design's plan of record | The PR diff touches only the file set the design's plan of record declares (recorded in the PR description as a path list), plus the spec/design/plan tree under `specs/1460-…/` |
