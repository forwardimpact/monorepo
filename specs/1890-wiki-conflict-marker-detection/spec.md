...
| 2   | The rule does not fire on marker strings inside backtick code spans or fenced code blocks in prose surfaces. The regression corpus includes a fixture reproducing the quoted-rider shape that produced the in-session false positive on #1668, anchored by content shape — not by source filename, which has already drifted once under seal rotation: a prose rider quoting both the branch-merge and stash-pop label forms in backtick code spans, including the line-wrapped close-marker variant (a quoted `>>>>>>> <sha>` landing at column 1 inside a span) and an in-span `=======`. A companion negative fixture quotes marker strings in straight-quote prose mid-line with no code span; line-anchored detection must not fire on these either. |
...
| 9   | The flow does not push commits that introduce an unresolved conflict block; the outcome carries a reason and local commits are preserved. Pre-existing corruption at origin does not block an unrelated writer's push. The regression corpus includes a dual-lineage fixture pinning the every-push binding (run-292 specimen, #1668 adjudication): two divergent lanes each introducing markers in their own outgoing commits, with **each** lane's push refused independently — the check is stateless across episodes and binds every publish attempt, not only the first detection.                                                                                                                                                                   |
...
- Shallow publish-leg field condition (grounds the Layer 2 shallow-checkout
  ...
  [#1564](https://github.com/forwardimpact/monorepo/issues/1564) occurrence #23,
  where a shallow clone boundary minted a falsified ledger entry that survived
  verification; (b) the 2026-06-12 (W24) #1668 repair verification, where the
  improvement-coach's boot clone at depth 2 could not resolve corruption-chain
  SHAs `a8be0c30`/`a95e1d57` until deepened — a shallow publish-leg checkout
  cannot self-audit its own corruption chain; (c) a live re-repro at the W24
  amendment-verification session boot, where the coach's clone was again shallow
  at depth 3 — depth varies per session — and both chain SHAs again failed to
  resolve. In every instance SHA resolution failed loudly
  (`fatal: Not a valid object name`); the hazard the constraint closes is a
  guard swallowing that failure as "no marker introduced". Routed by the coach
  via `wiki/improvement-coach-2026-W24.md` § [ask#1 repair-verification] and
  relayed in the facilitator's session broadcast; amendment fidelity verified by
  the coach in
...
- Dual-lineage every-push specimen (run 292, 2026-06-12): two divergent lanes
  ...
  [PR comment 4689999181](https://github.com/forwardimpact/monorepo/pull/1673#issuecomment-4689999181).
  Grounds criterion 9's dual-lineage fixture.
