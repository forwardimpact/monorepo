...
- **Shared audit posture — the #1185 precedent.** The previous stray-file
  ...
  24 of the bare shape — 23 title-cased, e.g. `# Staff Engineer — 2026-W23`,
  and one slug-cased: `staff-engineer-2026-W23-part3.md` opens
  `# staff-engineer — 2026-W23`). A strict cut-over to the new grammar
...
| Audit heading validation (shape-only) | Sealed-part heading validation additionally accepts the new `(part N)` shape. Every shape it accepts today — legacy `(part N of M)` and the bare suffix-less heading — remains accepted, so no existing sealed part gains a finding; structurally broken headings that fail today (missing separator, bad week token, malformed suffix) keep failing. The agent-prefix consistency check derives the agent title from all accepted shapes and matches by slug, not verbatim casing — the live tree's accepted parts carry both title-cased and slug-cased H1 titles. |
...
| Existing sealed parts pass the audit on day one. | Run `fit-wiki audit` over a fixture holding both legacy shapes — `(part N of M)` and bare suffix-less headings — mirroring the live tree at HEAD, including its casing variance: a bare title-cased part and a bare slug-cased part (`# staff-engineer — 2026-W23` on a `staff-engineer-…` filename); observe zero heading-shape and zero agent-prefix findings. |
...
| The agent-prefix consistency check covers all accepted shapes. | Audit one part of each accepted shape whose H1 title does not match its filename agent prefix; observe each is flagged. Audit a part whose H1 title differs from its filename prefix only in casing and word separators (slug-equal, e.g. `# staff-engineer — …` and `# Staff Engineer — …` on the same prefix); observe neither is flagged. |
...
| One contract home, hint in agreement. | The memory-protocol weekly-log contract section states the sealed-part heading grammar next to the filename convention it names, the audit hint names the same grammar, and the re-bisection path's documentation describes the H1 number as the filename slot — no surviving "local to this split" caveat. Here "weekly-log contract section" resolves to the shared grammar section of spec 1760 § Decision 2: the grammar is declared exactly once in memory-protocol.md, in that section, adjacent to the weekly-log filename convention, and the existing `## Weekly Log Contract` section references it without carrying a second copy. |
