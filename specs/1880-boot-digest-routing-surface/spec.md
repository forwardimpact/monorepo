...
staff-lane datum adds the first consequence-grade instance, an SE-lane datum
adds a second whose delay was recovered same-day, and a 6/12 pre-window
specimen shows the same failure mode recurring on the successor experiment the
day before its window:
...
| The realized cost extends to labeled-issue-only commitments, where no carry bullet exists and routing level 2 is the *sole* designed deliverer — Exp #1481's schedule-pinned Day-1 backfill (window 6/08–6/09) entered no working set across six completed audit-lane runs, executing ≥10 days late. Supersedence: once the binding was cross-linked onto #1481 itself (the dual-read-path mitigation mirroring RE's #1625 move), SE executed it in full the same day, 7 days ahead of the re-pinned horizon and method-upgraded — the sidecar CSV is now emitted, making this a *recovered* delay, distinct from the staff-lane row's permanent loss. The constraint was never lane capability; it was delivery, and the severity gradient (silence → recovered delay → permanent loss) is design-corpus signal | SE-lane instance datum [#1666 issuecomment-4690705143](https://github.com/forwardimpact/monorepo/issues/1666#issuecomment-4690705143); slip report [#1481 issuecomment-4690532544](https://github.com/forwardimpact/monorepo/issues/1481#issuecomment-4690532544); same-day execution record [#1481 issuecomment-4690811668](https://github.com/forwardimpact/monorepo/issues/1481#issuecomment-4690811668) |
