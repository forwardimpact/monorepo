...
| `websites/fit/gear/index.md:29-30`              | The sentence `Every library ships a matching skill in the `forwardimpact/fit-skills` pack` stops being true when the runtime skills move. Narrow it to `Most libraries ship a matching skill in the `forwardimpact/fit-skills` pack.` Do not name `gemba-skills` here: the page never installs it, and naming a pack its § Getting Started omits trades one wrong statement for another. Do not enumerate libraries either. The catalog is larger than the skill set and any list drifts. |
...
Leave `websites/fit/gear/index.md:9` and `:180`. The subtitle and the
§ Getting Started install block name `forwardimpact/fit-skills`, which stays
Gear's own pack and stays correct. Adding the new pack there is the same class
of edit the plan declines on four other pages. Only lines 29-30 change, because
this change makes that sentence false.
