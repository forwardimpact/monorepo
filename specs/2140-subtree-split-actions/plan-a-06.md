...
| D | Enum consumer fences | `CLAUDE.md` (`:list`, brace-expansion form), `KATA.md` (`:count`+`:list`) | reseed with the repository's enumeration-drift seed command, not by hand; verified by the enum-drift invariant (and, for the `KATA.md` `forwardimpact/fit-*` lines, the residual grep too) |
