...
| 9   | The four siblings are debranded and every monorepo sibling-name surface is repointed; `kata-agent`, CLIs, npm packages, and skill repos keep their names | `forwardimpact/{harness,benchmark,wiki,bootstrap}` resolve and `forwardimpact/fit-*` redirect to them; a repo-wide grep for `forwardimpact/fit-{harness,benchmark,wiki,bootstrap}` returns only the intentionally-stale set (`specs/**`, `**/CHANGELOG.md`, the synthetic enum-drift fixture); the `fit-*`/`kata-*` CLI bins and npm/skill repo names are unchanged; the enum source + consumers show the new names |
