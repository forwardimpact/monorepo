...
| 4   | The consumption mechanism is unchanged and no gitlink is introduced                                                                                  | `uses:` lines still resolve to sibling SHAs carrying the `# v1.0.x` (`# v1`-family) marker; `test ! -f .gitmodules`; Dependabot still bumps the sibling SHA-pins in the monorepo's workflows |
...
| 9   | The four siblings are debranded and every monorepo sibling-name surface is repointed; `kata-agent`, CLIs, npm packages, and skill repos keep their names | `forwardimpact/{harness,benchmark,wiki,bootstrap}` resolve and `forwardimpact/fit-*` redirect to them; a repo-wide **`--hidden`** grep for `forwardimpact/fit-{harness,benchmark,wiki,bootstrap}` returns only the intentionally-stale set (`specs/**`, `**/CHANGELOG.md`, the synthetic enum-drift fixture); the named bare-name files carry no action-identity `fit-*`; the `fit-*`/`kata-*` CLI bins and npm/skill repo names are unchanged; the enum source + consumers show the new names |
...
exercises once and every later push re-exercises. Criterion 9 is **split**: the
in-monorepo repoint and the residual grep are verified in the PR gate; the
maintainer repo-rename and the `forwardimpact/fit-*` redirect resolution are
verified at migration time, like the seed.
...
| Action repo naming                                   | **Debrand to bare functional names** where the bare name names a concrete artifact on its own (`harness`, `benchmark`, `wiki`, `bootstrap` each denote a specific thing a CI action does); the `forwardimpact/` owner already namespaces a published action, so `fit-` is redundant. Keep `kata-agent` — `agent` denotes a role, not an artifact, so it is too generic to stand alone. CLIs/npm/skills keep `fit-*`/`kata-*` (shell-global namespacing); action↔CLI name parity is dropped deliberately. Use GitHub repo-rename for redirect continuity |
