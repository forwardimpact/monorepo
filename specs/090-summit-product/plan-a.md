...
7. **No TypeScript** — the monorepo is JS with JSDoc types, matching the rest of
   `products/`.
8. **libskill subpath imports.** Import libskill functions from their subpath
   modules, not from the root. Match Pathway's convention
   (`products/pathway/src/formatters/skill/shared.js:11` and friends):

   ```js
   // Good — matches Pathway convention, tree-shakeable
   ...
   libskill's own `src/index.js` header documents the subpath imports as the
   preferred entry point (lines 6–15). Root imports now work (see libskill fix
   landed alongside this plan) but are strictly less preferred. A
   single-location convention also prevents future regressions where a
   root-export typo breaks Summit silently.
