...
| --- | --- | --- |
| `fit-benchmark` CLI | `libraries/libeval/bin/fit-benchmark.js` (new) | Entry point. Parses subcommand (`run`/`invariants`/`report`), wires real dependencies, delegates to the matching command handler. Mirrors `bin/fit-eval.js` shape. Same `@forwardimpact/libeval` package; separate bin. |
...
| `Task` | same module | One task: `id` = METR-style `task_family_name/task_name`, paths to `instructions.md`, `supervisor.task.md`, `judge.task.md`, `specs/`, `workdir/`, `invariants/`. |
...
| `WorkdirManager` | `libraries/libeval/src/benchmark/workdir.js` (new) | Per-task: creates a temp CWD, copies `workdir/` and `specs/` into it (never `invariants/`), copies the apm staging tree into the CWD's `.claude/`, runs the pre-flight smoke probe, owns process-group cleanup at teardown (port free, processes reaped via process-group SIGTERM then SIGKILL after a 5 s grace). Returns a `Workdir` handle. |
| `Workdir` | same module | `{ cwd, port, pgid, scaffold, agentTracePath, judgeTracePath, preflightError? }`. Threaded through `runAgent` → `invariants` → `judge` → `teardown`. |
| `Invariants` | `libraries/libeval/src/benchmark/invariants.js` (new) | Spawns `<template>/tasks/<task>/invariants/run.sh` from the **template** path with `node:child_process.spawn` using `stdio: ['inherit', 'pipe', 'pipe', 'pipe']`. The 4th stdio entry is the parent-readable pipe for NDJSON `{ test, pass, message? }` rows; the script writes to fd 3 (`$RESULTS_FD=3`). `$WORKDIR` env is the post-run agent CWD. Returns `{ verdict, details, exitCode }`. **Exit code is authoritative**: `verdict = "pass"` iff `exitCode === 0`. The fd-3 NDJSON is diagnostic (per-test breakdowns surfaced on the result record) and never overrides the exit-code verdict. |
| `Judge` | `libraries/libeval/src/benchmark/judge.js` (new) | Wraps a libeval `Supervisor` over a single `AgentRunner` with its own `TraceCollector` writing to `Workdir.judgeTracePath`. Supervisor task = the family's `judge.task.md`; agent task = a structured "produce a verdict" prompt with `invariantsPath` and `agentTracePath` exposed via env. `Conclude` is registered only on supervisor/facilitator tool servers (`orchestration-toolkit.js:224,311`); a bare `AgentRunner` cannot emit it, so the wrapper is required. The supervisor's `Conclude` arguments (`{verdict, summary}`, where `verdict ∈ {"success","failure"}`) are mapped into the result record by the runner: `judgeVerdict = { verdict: conclude.verdict === "success" ? "pass" : "fail", summary: conclude.summary }`. |
...
| Subcommand handlers | `libraries/libeval/src/commands/benchmark-{run,invariants,report}.js` (new) | Parse CLI args, validate paths, build `BenchmarkRunner` or `ReportAggregator`, write output. Mirrors existing `commands/run.js` shape. |
...
```mermaid
graph TD
  CLI[fit-benchmark CLI] --> CMD[run / invariants / report handlers]
  ...
  SC -. "invariants/ stays in template" .-> SC
  SC --> RR
  JD --> RR
...
sequenceDiagram
  ...
  loop task × runIndex (serial)
    ...
    BR->>SC: invariants(task, workdir)
    SC-->>BR: { verdict, details, exitCode }
    BR->>JD: judge(workdir, invariants) [trace → judgeTracePath]
...
| `verdict` | `"pass" \| "fail"` | combined gate: `"pass"` iff `invariants.verdict === "pass"` AND `judgeVerdict.verdict === "pass"` |
| `invariants` | `{ verdict, details, exitCode }` | `verdict` ∈ `{"pass","fail"}`; `details` parsed from `$RESULTS_FD` |
...
| `preflightError?` | `{ phase, message, exitCode }` | present only on pre-flight failure; the record is otherwise minimal (`costUsd: 0`, no submission, no invariants) |
...
Task: {
  id, paths: { instructions, supervisor, judge, specs, workdir, invariants },
}

// invariants.js
runInvariants(task, workdir): Promise<{
...
| --- | --- | --- | --- |
| 1 | Separate CLI `fit-benchmark` rather than a `fit-eval bench` subcommand. Same package. | Add `bench` to `fit-eval`. | Keeps `fit-eval` a low-level generic tool with stable surface. The benchmark layer carries opinionated semantics (lifecycle, invariants, judge, aggregation) that don't belong in the generic CLI. |
...
| 3 | Hidden `invariants/` directory lives only in the template; `WorkdirManager` never copies it. The Invariants invokes scripts from the template path with `$WORKDIR` as an argument. | Copy `invariants/` to a sibling dir; rely on the supervisor to keep the agent away. | Structural beats procedural. If `invariants/` is never on disk in the agent's CWD, peeking is impossible. |
...
| --- | --- |
| `benchmark/*.js` unit | `WorkdirManager` excludes `invariants/`. `ApmInstaller` produces stable `skillSetHash` and runs once per family. `Invariants` parses fd-3 NDJSON and exit codes. `validateResultRecord` rejects malformed records. `ReportAggregator` computes pass@k. |
| End-to-end fixture | A minimal task family driving the full lifecycle, asserting result-record validation, invariants isolation (sentinel-filename property), pre-flight failure path, JSONL append integrity, and verdict-enum mapping (`success`→`pass`). |
...
cost caps, replay-from-trace, intermediate invariants, concurrent runs,
per-task tool/network permissions, and Windows invariants-channel support are
