...
- `createMockSubprocess({ responses = {} } = {})` — returns `{ run, spawn, calls }`. `run(cmd, args, opts)` resolves to `{ stdout, stderr, exitCode }`, consulting `responses[cmd] ?? { stdout: "", stderr: "", exitCode: 0 }`. `spawn` returns an `{ stdout: AsyncIterable, stderr: AsyncIterable, exitCode: Promise, kill }` quad backed by an in-memory queue.
