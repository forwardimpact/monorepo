...
One `runtime` bag flows from each binary's entry point through libcli's
existing `InvocationContext.data` slot into every constructor and factory
...
| `LibwikiCommands` | libwiki (`src/commands.js`) | Single class; one method per CLI subcommand (`boot`, `audit`, `log`, `claim`, `release`, `push`, `pull`, `refresh`, `init`, `memo`, `inbox`, `rotate`, `fix`). Spec § Scope enumerated 12 handlers; this design extends to 13 to match the bin's shipped command set (`fix` post-dates the spec). Constructor `{ runtime, wikiSync }`. Each method takes `(ctx)` from libcli's dispatch and returns the result envelope below. |
...
flowchart LR
  ...
  entry -->|"cli.dispatch(parsed, { data: { runtime } })"| cli["libcli.Cli"]
  cli -->|"handler(ctx) = cmds.method(ctx)"| cmds
  cmds -->|ctx.data.runtime| domain["new Service({ runtime })"]
...
| 3 | Runtime flows through libcli's existing `InvocationContext.data` slot. Handlers receive `ctx` (per `cli.dispatch`'s current contract — `libraries/libcli/src/cli.js`) and read `ctx.data.runtime`. Bin shims call `cli.dispatch(parsed, { data: { runtime } })`. libwiki's bin migrates from its hand-rolled `COMMANDS` switch to `cli.dispatch` — the CLI definition gains `handler` fields per subcommand (non-trivial rewrite, scheduled in the libwiki PR). | Mutate `cli.runtime`; add a fourth handler parameter; new context type. | `ctx.data` is the documented host-injected slot (`libraries/CLAUDE.md § Invocation context`); reuse the channel that already exists. |
...
| 8 | Migration order: **libmock** (add subprocess + finder + git + gh fakes) → **libcli** (extend `createCli` to accept `{ runtime }` so `Cli.error`/`usageError`/help write through `runtime.proc.stderr` instead of the captured global; existing positional signature kept as a deprecated alias for one migration cycle) → **libutil** (Finder + GitClient + GhClient + `createDefaultRuntime` + `scripts/capture-cli-golden.mjs`) → **libconfig** (`process` arg → `runtime.proc`) → **libstorage** → **libwiki** (LibwikiCommands + WikiSync + bin rewrite to `cli.dispatch`) → **libeval** → **librpc** → **products** → **services**. STATUS carries one row per library — `1370/libutil`, `1370/libwiki`, etc.; this extends the `id` field to permit a `/<library>` suffix and requires `fit-wiki claim`, `kata-dispatch`, and `kata-release-merge` to parse the suffix as a sub-row of spec 1370. Spec 1370's master row advances to `plan implemented` only when every sub-row is. Spec § Risks proposed `libutil → libmock`; the design inverts because `libraries/libutil/package.json` already devDepends on libmock. The async-only subprocess decision (§ Surfaces) means the libwiki PR converts every libwiki sync caller chain — `commands/sync.js`, `commands/init.js`, and any consumer threading through `WikiRepo` — from sync to async. | Single 1370 row + deny-list as sole tracker; new sub-spec dirs; single monolithic PR. | Per-library rows honor the spec mitigation, make claims visible to `fit-wiki claim`, and let libraries advance independently. |
