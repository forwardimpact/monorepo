...
commands collapse onto a `LibwikiCommands` class so every command is a
method on a class constructed with explicit collaborators (Success
Criterion 4); the only `new Finder(...)` call site lives in libutil
(Success Criterion 9). A two-list lint check governs new code and
shrinking debt; STATUS carries one row per library/product (spec §
Risks mitigation).
