...
| The implementation PR's diff stays within the file sets in scope. | The PR diff touches only `.claude/skills/kata-release-cut/SKILL.md`, `references/` files under `.claude/skills/kata-release-cut/`, and the spec/design/plan tree under `specs/1500-kata-release-cut-hazards/`. The `references/` allowance exists so the implementation can displace content to the reference tier when the combined-budget constraint tracked on [Issue #1613](https://github.com/forwardimpact/monorepo/issues/1613) requires it; hazard letters still resolve in `SKILL.md` per the criterion above. |
