# Spec 830 — `fit-summit what-if --move` shows impact on both teams
...
- **Original product specification (`specs/090-summit-product/spec.md`,
...
| CLI `--help`                                    | wherever `fit-summit what-if` libcli definition lives   | Help text for `--move`/`--to` makes clear that the positional team arg is the source team and the diff covers both teams.                                                                 |
...
- **A `Net:` narrative summary.** The original spec 090 example showed a
...
| 2  | The JSON output for a `--move` scenario carries the source team's diff and the destination team's diff under named keys that identify which team is which; both `teamId` values are present in the output.                                            | Integration test: invoke the JSON formatter on the same fixture; parse the result; assert the document carries entries for both `<src>` and `<dst>` such that a reader can attribute a `capabilityChanges` array to each team without consulting the original CLI invocation.                                                                                                                      |
...
| 4  | Non-move scenarios produce byte-identical output to the pre-change implementation for all three formats.                                                                                                                                                | Snapshot test: capture text/JSON/markdown output for representative `--add`, `--remove`, and `--promote` scenarios on a fixed fixture before the change merges; assert post-change output equals the captured output for each.                                                                                                                                                                      |
...
| 7  | The destination-team computation reuses the same `mutated` roster the command already produces — there is no second mutation pass and no second `applyScenario` call.                                                                                  | Static inspection: in `runWhatIfCommand` (or its replacement) the destination snapshot is computed from the same roster value used for the source `after` snapshot. Reviewable in the diff; design and plan must preserve this property.                                                                                                                                                            |
...
- Original product spec example: `specs/090-summit-product/spec.md` 377–392.
