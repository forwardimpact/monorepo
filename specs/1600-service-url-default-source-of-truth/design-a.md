...
| Manifest declaration | Each service declares the keys libconfig needs to produce its listen URL; the spec leaves the per-service shape (`url` string vs. `protocol`+`host`+`port`) open, and the design preserves that — the registry records whichever form each service uses. | Mandating one uniform shape across all services — needlessly narrows a choice the spec defers per-service and conflates http/grpc handling. |
