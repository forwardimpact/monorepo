...
the lockfile. Issue #1458 itself names "drop the `restore-keys`
fallback" as remediation #1, which this spec adopts as its structural
fix.
...
### Before/after gate semantics

Today, the `Restore environment cache` step declares both `key` and
`restore-keys`. `actions/cache`'s documented behavior on a
`restore-keys` (prefix) hit is to **restore files to disk while leaving
the step output `cache-hit` as `false`**. So under today's contract,
`cache-hit != 'true'` does not mean "empty environment" — it means
"exact-key miss, possibly with a partial restored tree." That is
exactly the mode Issue #1458 records: a prefix-key hit restores a
divergent tree, `cache-hit` reads `false`, and the install step
reconciles in place against a partially-resolved layout.

With `restore-keys` removed, an exact-key miss yields no file
restoration at all. The install step at `cache-hit != 'true'` then
runs against an empty environment and resolves the lockfile from
scratch — the gate semantics the consumer-impact section describes
become true only after the contract change ships.
...
This spec depends on spec 1310's **implementation pass for
`fit-bootstrap` specifically** having shipped first. The monorepo
...
Once 1310's implementation pins `fit-bootstrap` to a SHA, advancing
...
`forwardimpact/fit-bootstrap` across 19 workflow files under
`.github/workflows/` resolve to `@v1` (mutable tag). Spec 1310's
`spec` and `design` are merged but no implementation pass has pinned
`fit-bootstrap` to a SHA in the monorepo. The sequencing above
treats that gap as a precondition this spec's implementation must
satisfy before opening its monorepo PR, not as a separate sibling-side
audit.

### Why this spec follows the immediate prefix bump

The release engineer is shipping an in-place cache-key-prefix bump on
the action (`env-v2-` → `env-v3-`) to evict today's poisoned cache and
unblock PR #1457 on the next CI run. That bump is the right immediate
move: it clears the visible symptom without authoring over the
structural answer. This spec is the follow-up that keeps the next
lockfile-changing bump from re-running the same thread.

Implication for success criteria: this spec's contract-change commit
on the sibling repo is **not** the diff against today's `env-v2-`
state; the interim `env-v2- → env-v3-` prefix bump may land between.
Each success criterion is evaluated against the contract-change
commit, not against current-main.
...
- Advancing every `forwardimpact/fit-bootstrap` SHA pin in the
  ...
  implementation pass for `fit-bootstrap` specifically establishes,
  comprising 28 references across 19 workflow files at the time of
  this spec — to a sibling-repo commit that carries the contract
  change, so the monorepo CI actually consumes the new contract.
...
- **The action's cache-write path.** Removing the fallback restore is
  ...
  the same key it computes today. The `actions/cache` post-step
  uploads the workspace tree under the resolved key when the job
  succeeds, so a cold-cycle run with no prior exact-key hit installs
  from the lockfile, succeeds, and uploads a tree resolved entirely
  from the lockfile under the new exact key. The next PR with the same
  exact key now restores a clean tree; the cross-cycle-poisoning loop
  closes without modifying the write path.
- **The cache-key composition.** The `hashFiles(...)` argument set on
  the `env-cache` step continues to hash the same inputs it hashes
  today; the change is in fallback semantics, not in the
  content-addressing inputs. (The literal `env-vN-` prefix may bump
...
on any key change, no files are restored, and the install step runs
against an empty environment and resolves the lockfile from scratch.
Cold-cycle install time on lockfile-changing PRs is the trade for
cross-cycle tree integrity — expected magnitude of the order of
+30–90s per affected job (lockfile-changing PRs only; the warm-key
path is unchanged). The action's inputs, outputs, and step ids are
otherwise unchanged.

## Success criteria

Each criterion is evaluated at the contract-change commit on the
sibling repo (or, for the monorepo-side criterion, at the merge commit
of this spec's monorepo PR). Criteria are not evaluated against
current-main; an interim `env-vN-` prefix bump may land between today
and the contract-change commit and does not affect any criterion below.
...
| The cache-key content-addressing inputs on that commit are unchanged. | At the same commit, the `hashFiles(...)` argument set inside the `key` input on the `env-cache` step is the same set the pre-change action hashes: `scripts/install-deps.sh`, `bun.lock`, `**/*.proto`, `libraries/libcodegen/src/**`, `libraries/libcodegen/templates/**`, `libraries/libcodegen/bin/**`. (The literal `env-vN-` prefix value may differ from current-main per the immediate prefix-bump move; it is not constrained by this criterion.) |
...
| The monorepo's references to `forwardimpact/fit-bootstrap` consume the contract change. | After this spec's monorepo PR merges, every workflow `uses:` reference to `forwardimpact/fit-bootstrap` under `.github/workflows/` — the set comprising 28 references across 19 workflow files at the time of this spec, advanced from `@v1` to the SHA pins spec 1310's implementation pass for `fit-bootstrap` specifically establishes — resolves to a sibling-repo commit that satisfies the first three criteria above. |
