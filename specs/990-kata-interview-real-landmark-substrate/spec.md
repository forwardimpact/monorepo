...
| Persona corpus | The substrate-prep step selects a single persona (the *chosen persona*) from the `human`-kind people seeded into `organization_people`. The seeding flows from the synthetic content under [`data/synthetic/`](../../data/synthetic/) through whatever ingestion path the design picks. The chosen persona must, in the seeded substrate, (a) be the `manager_email` of at least one other `organization_people` row (so commands taking `--manager <persona-email>` resolve to ≥1 row), (b) appear as the `email` of at least one row in the table `evidence` reads from (so `evidence --email <persona-email>` returns ≥1 row), and (c) be the `manager_email` of at least one row in the table `practice` reads from (so `practice --manager <persona-email>` returns ≥1 row). The corpus must also carry at least one organization snapshot id and at least one driver/item id, both surfaced through the discovery vector below. Which specific synthetic personas satisfy these properties is a design choice. |
...
| Three named row-class smokes return non-empty payloads attributable to the chosen persona. | The same CI smoke-test step parses `--format json` output for three specific commands — `org team --manager <persona-email>` (roster row class), `evidence --email <persona-email>` (evidence row class), and `practice --manager <persona-email>` (practice-pattern row class) — and exits non-zero unless each top-level row collection is non-empty *and* every row in each payload carries the chosen persona's email or `manager_email` in the field the command filters on. The corpus invariants in the Persona-corpus scope row are what make each smoke satisfiable. |
...
| Non-Landmark interviews are not regressed. | A CI step parses [`kata-interview.yml`](../../.github/workflows/kata-interview.yml) on the implementation branch and exits non-zero unless every step introduced by this spec's implementation, and every key added to the `Run interview` step's `env:` map, carries a step-level `if:` predicate that evaluates to false when `inputs.product != 'landmark'`. The predicate's exact form is a design choice. |
