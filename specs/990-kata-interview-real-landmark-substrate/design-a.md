...
1. **Env materialization.** Two existing seams populate the env. `just env-reset` copies `.env.local.example` (which carries `SUPABASE_URL=http://127.0.0.1:54321`) to `.env`. `just env-setup` then writes the seven secrets generated by `scripts/env-setup.js` (`SUPABASE_JWT_SECRET`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, plus `SERVICE_SECRET`, `DATABASE_PASSWORD`, `MCP_TOKEN`, `AWS_*`) into the same file. A subsequent invocation in `--add-mask --output` mode emits `::add-mask::` lines for the secret values so subsequent transits through step output are auto-redacted. `SUPABASE_URL` is documentation, not secret, and is not masked.
...
6. **JWT mint.** `fit-map auth issue --email <persona>` mints a JWT against `SUPABASE_JWT_SECRET`. The CLI's stdout interleaves header text + the bare JWT line + a `Export:` bullet + a success line (`auth-issue.js:87-98`); extracting the JWT is a single-line shell post-process. The extracted JWT is registered with `::add-mask::` before any subsequent step output transit, then written to `$GITHUB_OUTPUT`.
7. **Discovery file.** An inline step writes a JSON file with shape `{ persona_email, manager_email, snapshot_id, item_id }` into the workspace-prep step's `agent-cwd` directory (`steps.agent-workspace.outputs.dir`, the same path the existing workflow uses to stage `data/pathway/` and `data/activity/`). No JWT in the file.
8. **Agent dispatch.** The `Run interview` step gains a value-level conditional `env:` entry: `LANDMARK_AUTH_TOKEN: ${{ inputs.product == 'landmark' && steps.mint.outputs.jwt || '' }}`. The substrate-derived agent-CWD path is already in `agent-cwd:`; the discovery file is visible to the agent at `$AGENT_CWD/.substrate.json` once inside the action.
