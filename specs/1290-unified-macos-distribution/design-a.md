...
| Info.plist | Bundle keys align with directory name | **Already aligned — no work** | `CFBundleExecutable=Outpost` correctly names the launcher binary; `CFBundleName`/`CFBundleDisplayName=Outpost` is the intended menubar identity the spec itself preserves (spec.md:419). No edit needed. |
...
| `products/outpost/pkg/build.js` | `--launcher` (compile Swift), `--app` (`buildApp()` re-assembles into `products/outpost/dist/`), `--pkg` (`buildPKG()`). Today `--pkg` implies `--app` (`build.js:155`), so `buildPKG()` depends on `buildApp()` running first. | `--launcher` kept (the brew lane already calls it via `build-app-product`). `--app`/`buildApp()` **removed**, and the `--pkg`-implies-`--app` coupling severed — `--pkg` now wraps a bundle the workflow has already assembled and staged, rather than assembling its own. |
...
| Fate of `pkg/build.js --app` | Remove `buildApp()` and the `--app` flag; `--pkg` consumes a pre-assembled bundle whose path the caller supplies. | Leave `--app` as a thin alias that shells out to `build-app-product`. Rejected: a shim that wraps the canonical path is exactly the fallback CONTRIBUTING § Clean breaks forbids; nothing else calls `--app`. |
...
| Whether to consolidate `publish-macos.yml` into `publish-brew.yml` | No. Keep the two workflows; wire the `.pkg` lane to the canonical builder in place. | Fold the `.pkg` job into `publish-brew.yml` (PR #1153's framing). Rejected: the spec marks workflow-split reorg out of scope beyond what unification requires; two tags already race on `gh release create` with idempotent create, and a merge is a larger blast radius than SC3 needs. |
| Install path (SC2) | Leave `/Applications/Forward Impact/fit-outpost.app` as-is. | Re-open the path choice. Rejected: it already unifies both channels (commit `c1716f71`); changing it would regress SC2, not advance it. |

## Interfaces

- **`build-pkg.sh <dist_dir> <version>`** — unchanged signature and body. The
  caller passes `dist/apps` as `<dist_dir>`, so `APP_PATH` (`:18`) resolves to
  the canonical `dist/apps/fit-outpost.app` and the `.pkg` lands beside it.
  `IDENTIFIER=team.forwardimpact.outpost` and the `/Applications/Forward Impact/`
  payload location are unchanged. `buildPKG()` in `build.js` is rewired to call
  it with this dir; whether `--pkg` still drives `buildPKG()` or the workflow
  calls `build-pkg.sh` directly is a plan choice — either severs the
  `--app`-implies coupling.
