...
- **Bug 2 (`× null` track)** — one right answer (omit when null); shipped in PR #878.
- **Bug 3 (double period)** — one right answer (route through the existing `ensurePeriod()` helper); shipped in PR #878.
...
| **`professionalTitle` shape** | The contract defines what `professionalTitle` is — *rank token* (e.g. `"Senior"`, `"Level I"`), *role-complete string* (e.g. `"Senior Software Engineer"`), or *either, distinguished by a marker the formatter inspects*. The decision is a design choice; the spec requires that **one** shape becomes authoritative and that all paths (`generateJobTitle` at `libraries/libskill/src/derivation.js:263`, the starter `levels.yaml`, the synthetic-prose pipeline emitter, the BioNova fixture, and any published agent-aligned engineering standards examples) agree on it. |
...
| **BioNova fixture parity** | The BioNova synthetic standard used in `kata-interview` either (a) is regenerated to comply with the contract, or (b) is left as-is and the formatter normalises so it renders correctly. v1 requires that re-running the user-testing repro (`npx fit-pathway job software_engineering J060` against the same BioNova data the interview ran on) emits a document with no duplicated role token and no `"You will works"` (or equivalent verb-agreement bug). |
| **Synthetic-prose pipeline alignment** | Whatever the contract says, the prose generator that produces standards from a DSL (`libsyntheticprose` and `syntheticgen`) emits compliant `professionalTitle` and `autonomyExpectation` values for newly-generated standards. The mechanism (prompt update, post-processing pass, schema-driven generation) is a design choice. |
...
- **Spec ID note.** Reusing or sequencing this spec with a future #874
  follow-up (e.g. a broader formatter audit) is the next spec's concern,
  not this one.
...
| Claim | Verification |
|---|---|
| The composition contract for `professionalTitle` is documented in one canonical location. | Test: a reader following the link trail from `products/map/schema/json/levels.schema.json` → authoring guide → starter `levels.yaml` lands on a single section that names the chosen shape (rank-only, role-complete, or marker-distinguished) and gives at least one compliant and one non-compliant example. |
...
| The BioNova fixture used in `kata-interview` renders correctly after the spec lands. | Test: re-running `npx fit-pathway job software_engineering J060` against the BioNova data referenced by the `kata-interview` skill produces a markdown document with the role title appearing exactly once at the top level and no broken verb-agreement substring. |
| The synthetic-prose pipeline emits compliant values for newly-generated standards. | Test: a fresh standard generated through the spec-named synthetic generator emits `professionalTitle` and `autonomyExpectation` values that pass the v1 enforcement point. |
