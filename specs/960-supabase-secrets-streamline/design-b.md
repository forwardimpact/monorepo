...
| `products/landmark/src/commands/login.js:103-134` | `createPkceStorage()` stays in-module (Landmark-local). `resolveAnonClient({config, createClient})` calls `createAnonClient({config, auth: {flowType: "pkce", storage: createPkceStorage()}, createClient})` — the existing `createClient` injection is forwarded into the factory's `createClient` parameter. Error wording points at `just env-setup`. |
