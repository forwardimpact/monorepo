...
- **Grade verdict** = `healthy ∧ gatesPass ∧ fullMarks` (vacuously true parts
  when no gate or scored rows exist — a row-less healthy cell passes; a task
  with no graders is a no-op, not a failure).
...
| Invariants collector | `benchmark/invariants.js` | Pure collector, no verdict of its own: returns `{details, exitCode, stderr?}`. Unparseable fd-3 lines stay in `details` as `parseError` rows and grade as malformed. |
...
| Record schema | `benchmark/result.js` | `invariants` shape drops `verdict`; **required** `grade: {verdict, gatesPass, score?, malformed?}` on the happy record — a pre-break record fails validation; optional `hiddenTests: {details, error?}` (present iff the task has a suite; `error` carries an engine crash); optional top-level `score` (0–1) — the effective judge-zeroed value `report` aggregates; preflight branch pins them `undefined`. |
...
| Hidden-test execution | Harness engine driven by the `tests/` layout | Hook-authored shell: every coding task re-implements staging, execution, exit-code plumbing, and pristine-baseline restoration; tangles structural and behavioral checking in one script; each copy can silently mis-grade. |
...
| Workdir restoration | Engine backs up collisions, unstages after grading | Leaving staged files — the judge sees non-agent files and may flag them as scope creep, corrupting the scope-discipline gate. |
