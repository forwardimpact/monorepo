...
**Hidden test suites** become a first-class libharness subsystem: a task
declares its hidden tests in a manifest, and the harness stages each test into
the agent's tree, runs it, converts the outcome into a row, and restores the
tree afterward. **Invariants scripts** shrink to what their name says:
structural checks (presence, shape, anti-tamper) emitted as rows via
`fit-trace assert`. Task authors stop hand-rolling test staging, execution,
and exit-code plumbing in shell.
...
| --- | --- | --- |
| **Hidden test suite** — `tests/manifest.yaml` + test files beside `hooks/` | libharness: stage each file into the agent CWD, run it, emit one row per test from its exit status, restore the tree | Behavioral checks the agent must never see |
...
| 2 | **Roles live in the row.** `gate: true` marks a gate check; a positive `weight` (default 1 when the key is absent) marks a scored check; `weight: 0` marks a diagnostic. `gate` and a positive `weight` are mutually exclusive on one row. Manifest checks declare the same fields and map one-to-one onto rows. |
...
| 9 | **Hidden test suites are declarative.** A task ships hidden tests as a `tests/` directory beside `hooks/`: a `manifest.yaml` naming each test file, its role (gate, or scored with a weight), and the command to run it. libharness stages each file, executes it, converts the exit status into one row, and removes what it staged. Hook scripts never copy or run test suites. |
| 10 | **Staging preserves trust.** Graded test files come only from the harness-owned `tests/` directory — an agent-editable copy never vouches for itself, and agent-added files cannot flip a manifest check. After grading, the harness restores the workdir to the state the agent left it, so the judge grades the agent's work, not the harness's scaffolding. |
| 11 | **Manifest failures are authoring failures.** An invalid manifest (bad YAML, missing file, role conflict, path escape) fails family load before any agent spend. At run time, a missing scaffold or a crashed/hung test process becomes a *failing* row with a message — never a silent skip, never minted marks. |
...
| 15 | **Clean break with full migration.** Every `invariants.sh` in `benchmarks/` and in the libharness test fixtures migrates to the row contract in this change; `implement-feature`'s hand-rolled test logic moves into a `tests/` manifest and its hook script is deleted; the judge template variable `{{INVARIANTS_RESULT}}` becomes `{{GRADE_RESULT}}` across all nine `judge.task.md` files. No compatibility mode, flag, or dual-channel shim ships. Pre-migration ledgers still render (records carry their verdicts) but are not comparable across the semantics break, and no comparison may span it. |
| 16 | **Leading example.** `implement-feature` (kata-skills family) becomes the worked scored task: a manifest whose gate check runs the pristine baseline suite and whose five scored checks are the hidden feature tests, with the scope-discipline judge unchanged. One task demonstrates gate + score + judge composing with zero task-authored shell. |
| 17 | **Documentation.** Every surface that states the old contract ("exit code is authoritative/is the verdict") states the new one: rows authoritative, roles table, exit code = script health, the `tests/` manifest layout, and when to author a hidden test versus a structural check. |
...
| libharness hidden-test engine | New: manifest discovery and validation in the family loader; stage → run → row → restore execution engine |
...
| `benchmarks/` (all three families) | Eight `invariants.sh` hooks migrate to structural row emission (no `FAIL`-driven exits); `implement-feature` converts to the manifest-driven leading example and loses its hook script; `fit-wiki/cli-fix` rewrites its nonstandard row shape to `test`/`pass`; all nine judge templates rename the variable |
| libharness test fixtures + suites | The four fixture hooks and every test asserting exit-code-derived verdicts move to the row contract; one new fixture task exercises a hidden-test manifest end-to-end; help-text goldens refresh where command descriptions change |
...
- **Test-reporter parsing** (TAP, JUnit XML) — a manifest check's grade is its
...
| 6 | Each manifest check stages its file, runs it, and yields exactly one row from the exit status; a missing scaffold or hung test yields a failing row, not a crash | Engine unit tests with an injected subprocess; timeout case |
...
| 8 | An invalid manifest fails `loadTaskFamily` before any agent session starts | Family-loader unit tests per validation rule |
...
| 14 | Docs updated: rows-authoritative contract, roles table, exit-code demotion, `tests/` manifest layout on every surface that stated the old contract | Scored-task and hidden-suite sections exist in the skill, the authoring reference, the CLI reference, the Run a Benchmark guide, and `benchmarks/README.md` |
...
the design covers the row contract, the hidden-test manifest and engine, the
grading and aggregation composition, the best-of-k estimator, the authoring
surface, and the full hook migration, and `kata-plan` follows approval.
