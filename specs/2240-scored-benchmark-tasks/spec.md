...
| 2 | **Roles live in the row.** `gate: true` marks a gate check; a positive `weight` (default 1 when the key is absent) marks a scored check; `weight: 0` marks a diagnostic. `gate` and `weight` never co-occur on one row — any combination, including `weight: 0`, is malformed, so a stray weight can never silently disarm a gate. Hidden-suite checks map one-to-one onto rows with their role taken from the filename convention. |
| 3 | **Exit code is script health only.** A nonzero invariants exit means the grader itself failed: the run records verdict `fail`, with score 0 on scored tasks — a crashed hook can never mint marks from the rows it happened to emit before dying. Check outcomes must never drive the exit code; a well-formed hook ends with `exit 0` unconditionally (early `exit 0` after a failing gate is fine — the gate row already carries the failure). |
...
| 8 | **Malformed fails loud.** A row with a missing or non-boolean `pass`, an invalid `weight` or `gate` value, any `gate`+`weight` co-occurrence, or an unparseable line counts as a *failing* scored check (at its own weight when valid, else unit weight 1), is tallied on the record, and is surfaced in the report. Silently dropping a defect could mint full marks from a broken hook. |
...
| 9 | Malformed and unparseable rows count as failing scored checks and surface in the report; any `assert` failure (invalid `--weight`, missing target file) emits a failing row before the nonzero exit | Derivation unit tests; assert command tests |
