...
exact pin) is stamped at publish time from the tag, so no checked-in file
changes per release. Launchers publish in the same workflow run as their
source — both source and launcher tarballs pass a pre-publish smoke gate
...
```jsonc
{
  ...
  // hand-written once to the repo's canonical shape; invariant condition (c) guards the placeholders
}
```
...
sequenceDiagram
  ...
  loop each launcher
    ...
    Job->>Job: assert installed @forwardimpact/<src> version == source version
  end
...
installed `@forwardimpact/<src>` version equals the source version
(Decision 6b).
...
| 6a | Publish step is **idempotent**: per package, `npm view <pkg>@<ver> version` first, skip if present | Tolerate `npm publish`'s `409 Conflict` by ignoring exit code | A pre-check is explicit and fails loudly on other errors; 409-swallowing hides distinct errors that share an exit code. The check must treat `E404` as "absent, proceed" (every launcher's first release) while still failing on other registry errors. |
| 6b | Smoke gate asserts **per-bin banner identity and resolved source version**: `--help` output contains the invoked CLI's bare bin name as a substring (e.g. `fit-trace`, not just `fit-eval`'s suite banner), and the `@forwardimpact/<src>` installed under the clean dir reports the source's `package.json` version | Trust exit code alone, or trust any non-empty stdout | The banner check catches an `import` that silently routes to a sibling bin — the spec's "banner names the specific bin, not a sibling" criterion. The version check catches the one stamp failure the install cannot: a wrong pin that matches an already-published version resolves from the registry and looks healthy; spec § Success Criteria row 6 requires the gate to fail exactly that skew. |
...
| Stamp step bug (wrong or missing version) | A pin to an unpublished version fails the clean-dir install; a pin to an already-published version resolves from the registry but fails Decision 6b's resolved-version assertion — either way the smoke gate blocks publish |
