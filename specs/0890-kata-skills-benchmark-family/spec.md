...
| The task carries the inputs the agent needs and hidden grading material that checks the agent's artefact against invariants against the skill's own contract. | Test: the v1 task's grading material lives outside the agent's working directory per spec #870's hidden-grading contract and asserts the rubric named in the In-scope table (the quality bar `kata-spec` publishes plus the judge's brief-addressed check). |
