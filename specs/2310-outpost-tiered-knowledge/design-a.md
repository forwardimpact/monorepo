...
the owner-only tier: the standard home for work in progress and for
permanent owner-only material. Two actions move content outward: promote (a
human moves a note into its tier) and export (a skill sends a copy through a
channel; the note keeps its tier). The tier order becomes a one-way link
rule. The whole instruction system (root CLAUDE.md, six agent profiles,
every skill) teaches placement by tier. `fit-outpost validate` gains
knowledge checks for rank grammar and uniqueness, link direction,
resolution, and format, path-string detection, and legacy-layout detection,
with machine-readable output and a findings baseline. The change is a clean
break and ships as the next major version with a MIGRATION.md playbook.

## Restated problem

One `Knowledge/` directory is one sharing unit with one audience, drafts and
permanent owner-only material have no home in the graph, and a naive folder
split leaks restricted titles through wiki links while dangling for partial
recipients. Success means: init creates the five default tiers at the root,
every instruction surface declares tier placement, the validator reports
rank, link, path-string, and legacy violations with file and line, emits
machine-readable findings on request, downgrades baselined findings to
warnings, passes conforming full vaults, suffix subsets, and symlinked
tiers, and the `Knowledge/` and `Drafts/` layout is gone from every template
and docs surface.
...
| Tier identity | A one-digit rank prefix; the tier set is the matching directories present | A tier manifest file — a second source of truth that a partial share can omit. Per-note front matter is excluded by the spec. A multi-digit grammar (`^[0-9]+-`) — date-prefixed personal folders such as `2026-Archive/` would become tiers. |
...
enforcement, sync tooling, encryption, redaction, per-entity share scoping
inside a tier, and the Outpost trust-boundary architecture stay untouched
per the spec's exclusions.
