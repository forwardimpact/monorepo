# Spec 970 — A Published Home for Shared Protos
...
| Repository-level proto-publication invariant | A new check, lint, or convention prevents the silent recurrence of this bug: a shared proto added in the future does not slip into an unpublished state. The form of the protection is a design choice. The observable property is that deliberately introducing a `proto/` import in any published package with no published home for the imported file fails some pre-merge or pre-publish gate, reproducibly. |
...
| Single editable source per shared proto. | Test: running `git ls-files '*tool.proto' '*resource.proto' '*common.proto'` from the repo root returns exactly one path per filename outside `generated/`. Files inside `generated/` are build output and are not counted; any other duplicate is either reproducible by re-running a documented build step (named in the design or plan) or is rejected by the recurrence guard below. |
...
| Recurrence guard. | Test: a deliberately introduced `import "missing-proto.proto";` line in any published package's `proto/` directory (with no corresponding file shipped in any published package) causes a pre-merge or pre-publish gate to fail with an error message naming the unresolved import. The check is reproducible by a reviewer on a test branch. The exact gate (CI step, lint rule, pre-publish hook) is a design choice. |
