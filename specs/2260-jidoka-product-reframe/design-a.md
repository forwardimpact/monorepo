...
| Standard rebrand               | `JIDOKA.md` (from `COALIGNED.md`)                                                                     | Same eight layers, layer rules, and length caps; framing rewritten around jidoka alongside the existing JTBD and Checklist Manifesto groundings; adds the downstream migration note (`.coaligned/` → `.jidoka/`, CLI and pack renames — an unmigrated repo gets a hard `rules directory not found` error naming the expected location, the loader's existing stop-the-line behavior). `MONOREPO.md`, `CLAUDE.md`, `CONTRIBUTING.md` references and the `.rgignore` entry naming `COALIGNED.md` (fixture-safety shield for the standard's example tags) follow.                                                                                                                                                                                    |
...
| Eval lane                      | `eval-jidoka.yml` (from `eval-coaligned.yml`), `benchmarks/jidoka-skills/` (from `coaligned-skills/`) | Workflow renames (name, concurrency group, `family:` path); the family's `apm.yml` dependency becomes `forwardimpact/jidoka-skills`; its hooks update end to end — the `preflight.sh` assertions on `.claude/skills/jidoka-{setup,jtbd}` directories and the `invariants.sh` greps for `jidoka invariants`/`.jidoka/invariants` — along with workdir fixtures, judge/task prose, and the `benchmarks/README.md` families table row. The pass@k series restarts under the new family name.                                                                                                                                                                                                                                                         |
