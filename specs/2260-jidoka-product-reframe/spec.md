...
| Standard rebrand               | `COALIGNED.md` → `JIDOKA.md`, rewritten framing (same layers, same rules) grounded in the Toyota concept, carrying the downstream migration note; `MONOREPO.md`, `CLAUDE.md`, `CONTRIBUTING.md`, skill/doc references, and the `.rgignore` entry shielding the standard's example tags from tag discovery follow.                                                                                                                       |
...
| Eval lane                      | `eval-coaligned.yml` → `eval-jidoka.yml`; `benchmarks/coaligned-skills/` → `benchmarks/jidoka-skills/`, updated end to end: pack dependency, task hooks (including the skill-directory assertions and config-directory greps), workdir fixtures, judge/task prose, and the `benchmarks/README.md` families table row. The pass@k series restarts under the new family name.                                                             |
...
`jidoka`, and the pack published at merge instructs `npx @forwardimpact/jidoka`
before step 2 has cut that package — the same expected window as spec 2250's
rename; execute same-day.

| #   | Step                                | What it does                                                                                                                                                                                                                         |
| --- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | Sibling repo operations (pre-merge) | `release-engineer`, in the authenticated `gh` environment: rename `coaligned-skills` → `jidoka-skills`; create `forwardimpact/jidoka` for the action split. The monorepo PR then merges with workflows already naming the new repos. |
...
| 5   | No surface invokes the old command name.                 | Static check: `rg --hidden --no-ignore -n coaligned` with explicit excludes for `.git/`, `node_modules/`, `specs/`, `wiki/`, and `**/CHANGELOG.md` (pinned so neither hidden dirs nor `.rgignore` skew the sweep) returns only (a) workflow SHA pins and installed-CLI lists held for release-train step 4 and (b) the migration note; root scripts and recipes invoke `jidoka` workspace-resolved.                           |
...
| 9   | The standard is rebranded.                               | Static check: `JIDOKA.md` exists with the migration note, `COALIGNED.md` does not, `MONOREPO.md`, `CLAUDE.md`, and `CONTRIBUTING.md` reference `JIDOKA.md`, and `.rgignore` lists `JIDOKA.md` in place of `COALIGNED.md`. Editorial review on the PR confirms the Toyota-concept framing.                                                                                                                                     |
...
| 11  | The launcher is retired without a successor.             | Static check: `launchers/coaligned/` does not exist, no `launchers/jidoka/` exists, the `public-cli-set` invariant passes with no `coaligned` or `jidoka` allowance, published skills and docs invoke `npx @forwardimpact/jidoka` or the bare binary, and the skill lint rejects bare `npx jidoka`/`bunx jidoka`.                                                                                                             |
...
| 14  | The eval lane exercises the renamed product.             | Static check: `eval-jidoka.yml` exists with family `./benchmarks/jidoka-skills`; `eval-coaligned.yml` and `benchmarks/coaligned-skills/` do not exist; the family's pack dependency names `forwardimpact/jidoka-skills`, its hooks assert `jidoka-*` skill directories and reference `.jidoka/invariants`, its workdir fixtures carry no `coaligned` token, and the `benchmarks/README.md` families table names the new pair. |
