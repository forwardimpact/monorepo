...
| `compare(other, {aIdentity, bIdentity})` | `{a, b, toolDelta, pathDelta}` (see below) | `other` is a peer `TraceQuery`; each of `aIdentity`/`bIdentity` is `{caseName, participant}` (basename-derived — the `TraceQuery` carries no filename), surfaced as that side's result `metadata`. See Key Decisions rows "`compare()` return shape", "`compare()` per-trace surface", "`compare` identity source" |
...
falling back to the basename with its trailing `.ndjson` extension stripped as
`caseName` (with `participant: null`) when a name doesn't match (`compare`
accepts arbitrary files). Only the final extension is trimmed — not a presumed
`.<role>.ndjson` tail, since a non-matching name carries no reliable role token.
`renderCompare` prints `participant` unconditionally (sentinel below), so the
null is visible in text, not silently dropped. The trace's own
