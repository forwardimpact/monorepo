...
| Scheduled wiki-curation audit | `.github/workflows/curate-wiki.yml` (new, `permissions: issues: write`) | Cron run of `fit-wiki audit` (read-only, no agent) against the live wiki; on findings, opens/updates one labeled issue addressed to the technical-writer. Sole home of the shared-state verdict. |
...
| Routing contract | The scheduled audit opens-or-updates one issue with a fixed title-key (so each cron tick updates rather than duplicates), via `GITHUB_TOKEN`; "naming an owner" is satisfied by a label and body text addressing the technical-writer, not a GitHub assignee (an App/agent profile is not a reliably assignable user) | A fresh issue per tick, or a `fit-wiki memo` from CI | A stable title-key gives the idempotent "single labeled issue" the success criterion checks. CI has no agent session to author a memo; the issue is the durable named-owner artifact. |
