...
|---|---|---|
| Identity-proof registry | `services/ghuser/src/identity-contracts.js` (new) | Surface → contract record `{evaluatedAt: "Begin" \| "Complete", evaluate({req, bridgeClient, flow}) → Promise<{outcome:"ok" \| "proof_missing" \| "identity_mismatch"}>}`. `bridgeClient` is the singleton collaborator injected at `GhuserService` construction (matches the existing collaborator-injection style). Adding a new surface requires registering one such record. Lookup miss returns the same `{outcome:"proof_missing"}` as a failed proof. |
...
| Bridge verification RPC | `services/bridge` `VerifyPendingDispatch(VerifyPendingDispatchRequest{link_token, expected_surface, expected_surface_user_id, tenant_id}) → common.Empty` | gRPC contract: returns OK iff the pending entry matches and is not already claimed. Errors as `NOT_FOUND` (no entry) or `FAILED_PRECONDITION` (mismatch or already claimed). `tenant_id` carried for proto idiom parity; ghuser supplies empty string. Pending lookups are not tenant-keyed today (`services/bridge/index.js:215` keys on `link_token` only), so the empty value matches the corresponding `PutPendingDispatch` write. Forward-looking invariant lives in Key decisions. |
| Claimed-dispatches index | `services/bridge` `claimed_dispatches.jsonl` (new, sibling to `pending_dispatches.jsonl`) | Append-only single-use ledger. **Invariant**: two concurrent verifies for the same `link_token` must not both observe "unclaimed". Architectural choice satisfying it: both `pending_dispatches` and `claimed_dispatches` are eagerly loaded at boot (`BufferedIndex.loadData` from `server.js` before `start()`), so the runtime check and the in-memory append on the handler are synchronous past the load-time await — gRPC handlers serialising on the event loop then guarantee no interleave (same invariant `ResolvePendingDispatch.compact()` already relies on, `services/bridge/index.js:234-239`). Independent of `pending_dispatches.jsonl` so the legitimate `/api/link-complete` consume is unaffected. |
...
| Atomic three-removal | `services/ghuser` | Single release tag retires three coupled gates plus their tests, with no intermediate state on `main`: (1) `BEGIN_ALLOWED_SURFACES` + kill-switch + `surface_not_supported` outcome, (2) `DISPATCH_ALLOWED_SURFACES` + `GetToken` quarantine, (3) `GITHUB_ID_SURFACES` + `Complete`-time identity-mismatch gate (folded into the registry's `github_account_equality` contract). Dependent tests rewritten in the same tag: `services/ghuser/test/{query-quarantine,identity-verification}.test.js` and the kill-switch row in `services/oauth/test/authorize.test.js`. Other ghuser query tests (`query-linked`, `query-reauth`, `query-unlinked`, `smoke`) audited for any `surface_not_supported`/surface-allowlist assertions and updated. `services/oauth` `outcome → HTTP` table is unchanged: `proof_missing` reuses the same generic `→ 503` path `surface_not_supported` already travels. |
