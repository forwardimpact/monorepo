...
is `bridge_pending_dispatch_proof`. **Spec amendment flagged**: the spec
does not name the conversation-type restriction; without it the proof
model is unsound. The design proceeds on the assumption the spec author
ratifies this tightening — if not, the spec returns to draft.

## Components

| Component | Where | Role |
|---|---|---|
| Identity-proof registry | `services/ghuser/src/identity-contracts.js` (new) | Surface → contract record. Lookup miss returns the same `{outcome:"proof_missing"}` as a failed proof. |
...
| Bridge verification RPC | `services/bridge` `VerifyPendingDispatch(VerifyPendingDispatchRequest{link_token, expected_surface, expected_surface_user_id, tenant_id}) → common.Empty` | gRPC contract: returns OK iff the pending entry matches and is not already claimed. Errors as `NOT_FOUND` (no entry) or `FAILED_PRECONDITION` (mismatch or already claimed). `tenant_id` mirrors the existing bridge-proto idiom; carrying an empty string is accepted as the single-tenant case. Multi-tenant tenant-keyed semantics are deferred — out of scope for this spec. |
| Claimed-dispatches index | `services/bridge` `claimed_dispatches.jsonl` (new, sibling to `pending_dispatches.jsonl`) | Append-only single-use ledger. `VerifyPendingDispatch` appends `{link_token}` on success; presence on subsequent lookup → `FAILED_PRECONDITION`. Independent of `pending_dispatches.jsonl` so the legitimate `/api/link-complete` `ResolvePendingDispatch` destructive consume is unaffected. Atomicity inherits the same single-instance-per-tenant event-loop serialisation invariant as `ResolvePendingDispatch`'s `compact()` (see `services/bridge/index.js:234-239`). |
...
| Pre-fix binding migration | `services/ghuser/src/migrations/` (new), invoked from `server.js` after stores load and before `server.start()` | Iterate `bindings.jsonl`; for each record whose surface's registry contract is `bridge_pending_dispatch_proof`, call `BindingStore.delete`. Records the run in `migrations.jsonl`. Idempotent — second boot reads the marker and skips. |
| msbridge personal-conversation gate | `services/msbridge` `#stashAndPostLink` | When `activity.conversation.conversationType !== "personal"`, skip `prepareLinkResume` / `putPendingDispatch` and post a static "DM the bot to link your account" message. The gate is the architectural mitigation for the link-token-confidentiality invariant in § Trust model. The msteams test suite gains a `personal-conversation-gate.test.js` covering personal, group, channel, and undefined-conversationType. |
...
| Bridge availability failure mode | Fail closed — any non-`OK` from `VerifyPendingDispatch` → `proof_missing` | **Fail open on transport error** — re-opens the original defect during bridge outages. Correct trade: link flow degrades to "retry later" rather than to "bound to attacker". |
...
| `tenant_id` plumbing | Bridge proto carries `tenant_id` for idiom parity; ghuser supplies empty string in the single-tenant default | **Thread `tenant_id` through `/authorize` query, `BeginRequest` proto, `prepareLinkResume`, and `msbridge` activity inspection** — cross-cutting plumbing larger than the spec scope; the spec does not name multi-tenant isolation as a goal. Deferred to a separate spec. |
