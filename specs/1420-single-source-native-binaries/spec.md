...
| Homebrew Outpost cask (`publish-brew.yml`, `outpost@v*`) | `outpost@v*` | special-cased in `build-app-product` to Outpost's own `just build`, which rebuilds the binary instead of using the shared `build-binary` Mach-O | zipped `.app` |
| Outpost installer (`publish-macos.yml`) | `outpost@v*` | Outpost's own `just pkg` build (same `pkg/build.js` compile) | `.pkg` |
...
| Bespoke `just` target cleanup | The per-category binary-build recipes spec 0600 introduced — `build-binaries`, `build-product-binaries`, and `build-gear-binaries`, each a hand-maintained enumeration (or aggregator of them) that drifts from the actual built set — are removed, along with the `build-apps` fan-out aggregator that chained them. The only binary-build entry point left is the single generic `build-binary` recipe, driven by one source-of-truth list of the build set; no per-product or per-category build recipe survives, and no surviving recipe references a removed one or carries its own build enumeration. The `.app`-assembly recipes (`build-app-product`, `build-app-gear`) keep their irreducible per-cask packaging configuration (bundle layout, plists, entitlements, resources, and Outpost's Swift launcher) but stop compiling — they consume already-built binaries: `build-app-gear`'s inline CLI list is replaced by the gear subset of the source-of-truth list, and `build-app-product` no longer compiles `fit-outpost` (it consumes the shared binary). Outpost's `.app` stays structurally distinct by necessity (its launcher is the bundle's primary executable); only the compile special-case is removed, not the launcher handling. Documentation that names the removed recipes (e.g. the release internals page) is updated. |
...
| The bespoke per-category build recipes are gone and `build-binary` is the only binary-build entry point. | Grep the `justfile`; observe `build-binaries`, `build-product-binaries`, and `build-gear-binaries` are absent, `build-binary` is the only binary-build recipe, and no surviving recipe references a removed one. |
| The distributable CLI set is defined in exactly one authoritative place. | Locate the source-of-truth list; observe both the CI build and any local "build all" derive their CLI set from it, and no recipe or workflow re-enumerates the set (including `build-app-gear`, whose inline list is gone). |
| The Outpost `.app` consumes the shared `fit-outpost` binary instead of compiling its own. | Read `build-app-product`; observe its Outpost path no longer compiles the `fit-outpost` scheduler and sources it from the shared build (the Swift-launcher bundling may remain). |
