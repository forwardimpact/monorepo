...
| Bespoke `just` target cleanup | The per-category binary-build recipes spec 0600 introduced — `build-binaries`, `build-product-binaries`, and `build-gear-binaries`, each a hand-maintained enumeration (or aggregator of them) that drifts from the actual distributed set — are removed. The only binary-build entry point left is the single generic `build-binary` recipe, driven by one source-of-truth CLI list; no per-product or per-category build recipe survives, and no surviving recipe references a removed one or carries its own CLI enumeration. Recipes that fan out over the old set (`build-apps`) and the `.app`-assembly recipes (`build-app-product`, `build-app-gear`) are retargeted to the generic build + the source-of-truth list and consume already-built binaries; `build-app-gear`'s own inline CLI enumeration is removed and `build-app-product` loses its `fit-outpost` special-case (its binary now comes from the generic build). Documentation that names the removed recipes (e.g. the release internals page) is updated. |
...
| The `.app`-assembly no longer special-cases `fit-outpost`. | Read `build-app-product`; observe no `fit-outpost`/`outpost` conditional branch — the Outpost `.app` consumes the generically-built binary like every other product. |
