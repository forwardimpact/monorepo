...
| CLI-set manifest (new, single source of truth) | One list of the distributable CLI names, read by `build-binaries.yml`'s matrix. Replaces the hand-maintained `build-product-binaries`/`build-gear-binaries` enumerations. | consumed by the matrix and the `.app`-assembly step |
...
| Where the CLI set lives | One manifest the matrix reads; the 0600 per-category recipes are deleted, not called from CI. Local "build all" becomes a loop over the same manifest. | **Keep `build-product-binaries`/`build-gear-binaries` and have the matrix call them.** Leaves two lists (justfile + matrix) to drift apart — the exact failure that let a broken `fit-codegen` sit in the gear list. |
| `.app` assembly after cleanup | `build-app-product`/`build-app-gear` survive as the macOS packaging step but lose the `fit-outpost` special-case (its binary now arrives from the generic build) and consume the manifest rather than re-enumerating. | **Fold `.app` assembly into the workflow YAML.** Larger blast radius than the spec's scope; assembly is unchanged behaviour, only its binary source moves. |
