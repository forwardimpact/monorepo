...
|---|---|---|
| `just build-binary NAME TARGET` (existing, now the **only** build recipe) | The sole compile primitive and sole binary-build entry point. Unchanged contract: resolve `NAME` to a package `bin` entry, `bun build --compile` it, inject the version via `--define`. The injected name is `NAME` upper-cased with `-`→`_` (so `fit-wiki` → `FIT_WIKI_VERSION`). The 0600 enumeration recipes (`build-binaries`, `build-product-binaries`, `build-gear-binaries`) are deleted. | in: bin name + Bun target → out: `dist/binaries/NAME` |
| CLI-set manifest (new, single source of truth) | A checked-in data file (e.g. `build/cli-manifest.json`) listing each distributable CLI with the metadata `.app` assembly needs — bundle membership (which product cask, or the gear bundle) and the per-cask plist/entitlement/resource keys. The one place the set is enumerated; see Key Decisions for why it is a file rather than a recipe. | read by `build-binaries.yml`'s matrix (names + targets), the `.app`-assembly recipes (membership + metadata), and the local build loop |
...
| `fit-wiki` version resolution | Add a `FIT_WIKI_VERSION` read (injected by `just build-binary`'s `--define`) with a source-execution `package.json` fallback — the pattern `fit-codegen` already uses. Today `fit-wiki` has no env branch at all. | **Bundle/read `package.json` from a fixed path in the binary.** Fights the virtual mount and diverges from the `FIT_<NAME>_VERSION` convention every other compiled bin follows. |
| `fit-outpost` entry + version | Shared build compiles the package's existing `bin` entry (the real entry that constructs the runtime and invokes it); rename its build-time version read from `OUTPOST_VERSION` to `FIT_OUTPOST_VERSION` (the name `just build-binary` injects), and update the matching `--define` in Outpost's `pkg/build.js`. Retire the `src/outpost.js` compile. | **Add a self-exec entry to `src/outpost.js`.** Duplicates the runtime construction site the codebase centralizes in the bin wrapper, leaving two entries to keep in sync. |
...
| `.app` assembly after cleanup | `build-app-product`/`build-app-gear` survive as the macOS packaging step but stop driving compilation: they `download`/read already-built binaries, take bundle membership + per-cask metadata from the manifest (retiring `build-app-gear`'s inline 25-CLI list), and drop the `fit-outpost` special-case. `build-apps` (which depended on the deleted `build-binaries`) is removed; its fan-out role is the matrix's. | **Fold `.app` assembly into workflow YAML.** Larger blast radius than scope; assembly behaviour is unchanged, only its binary source and enumeration move. |
