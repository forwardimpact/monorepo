...
|---|---|---|
| `just build-binary NAME TARGET` (existing) | The sole compile primitive. Unchanged contract: resolve `NAME` to a package `bin` entry, `bun build --compile` it, inject the version via `--define`. The injected name is `NAME` upper-cased with `-`→`_` (so `fit-wiki` → `FIT_WIKI_VERSION`). | in: bin name + Bun target → out: `dist/binaries/NAME` |
