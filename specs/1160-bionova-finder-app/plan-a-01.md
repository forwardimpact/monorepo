...
| `LICENSE` | Apache-2.0 |
| `.gitignore` | `node_modules/`, `products/finder/site/supabase/migrations/20250101000*_seed_*.sql` (staged copies — the vendored source `data/synthetic/seed/` is committed, see part 03 r2), `.env`, `*.log`, `infrastructure/*/data/` |
| `.nvmrc` | `20` (Node major) |
...
| `justfile` | Recipes: `up` (`docker compose up -d`), `down`, `setup` (`./setup.sh`), `seed` (`bash scripts/stage-seed.sh`), `cli` (`node products/finder/cli/bin/bionova-finder.js`), `dev:site`, `test`, `lint` — no `terrain` recipe: bionova-apps never runs `fit-terrain` (see plan-a.md § Approach) |
...
```json
{
  ...
  "scripts": {
    ...
    "lint:deno": "cd services/finder-functions && deno lint",
    "lint": "bun run lint:js && bun run lint:deno",
    "test:js": "bun test",
    "test:deno": "cd services/finder-functions && deno test --allow-net --allow-read --allow-env",
...
`services/finder-functions/` is a Deno project. libterrain is NOT a
...
| `finder-functions` | builds `services/finder-functions/` (placeholder Dockerfile) | 8082:8000 | `curl -f http://localhost:8000/health` | `kong`, `tei` |
...
**Build context for product services**: `finder-site` and
`finder-functions` need workspace-root access at build time (their
Dockerfiles `COPY products/finder/handlers …`). Set explicit build
...
```yaml
  finder-site:
    build:
      context: .
      dockerfile: products/finder/site/Dockerfile
  finder-functions:
    build:
      context: .
      dockerfile: services/finder-functions/Dockerfile
```
...
| `/functions/v1/*` | `http://finder-functions:8000` |
