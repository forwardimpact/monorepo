...
| `LICENSE` | Apache-2.0 |
| `.gitignore` | `node_modules/`, `products/polaris/site/supabase/migrations/20250101000*_seed_*.sql` (staged copies — the vendored source `data/synthetic/seed/` is committed, see part 03 r2), `.env`, `*.log`, `infrastructure/*/data/` |
| `.nvmrc` | `20` (Node major) |
...
| `justfile` | Recipes: `up` (`docker compose up -d`), `down`, `setup` (`./setup.sh`), `seed` (`bash scripts/stage-seed.sh`), `cli` (`node products/polaris/cli/bin/bionova-polaris.js`), `dev:site`, `test`, `lint` — no `terrain` recipe: bionova-apps never runs `fit-terrain` (see plan-a.md § Approach) |
...
`services/polaris-functions/` is a Deno project. libterrain is NOT a
