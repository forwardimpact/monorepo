...
| 5 | When the director-tier rollup output is projected to the rows belonging to a single team in its aggregation scope, the projection agrees with the existing `--manager <team-manager-email>` rollup for that team on every column the team-manager rollup produces, after both outputs are joined on a stable key the design names (e.g., `(team_id, driver_id, skill_id, quarter)` or analogous) | (a) capture the director-tier output for the IT department from criterion 1; (b) project it to rows whose team identifier equals one of the six IT teams; (c) capture the team-manager output for that team's manager via the existing `--manager` filter; (d) join the two outputs on the design-named stable key; (e) confirm every column the team-manager rollup produces holds equal values in both — director-tier output MAY carry additional aggregation-context columns (e.g., `% of department`, `scope`, tier label) but must not disagree on team-manager-produced columns |
