...
| Health view builder | `products/landmark/src/commands/health.js` | Detect the distinct `getdx_team_id` set in the score rows. With **1 team**: build `view.drivers` exactly as today (no shape change). With **≥2 teams**: build `view.teamRollup` — one entry per team `{ teamId, teamName, drivers[] }` where `drivers[]` is the existing driver-join applied to that team's score rows only — and a `view.scope` field `{ teamCount, tierLabel }`. The flat `view.drivers` array is omitted under rollup so no driver row double-counts. |
...
| Evidence + Summit recommendations stay union-scoped, not per-team | These are per-skill across the resolved member set (the union of six teams), exactly as the single-team path scopes them to one team's members. They are not rollup rows, carry no team or person identifier in the rollup, and are unaffected by SC-5 (which constrains only the per-team driver rows). The director sees the same recommendation surface a manager sees, scoped to the wider set. |
