...
//    invoke fit-* CLIs with npx.
// 2. Monorepo identifiers and snapshots — the @forwardimpact workspace
//    scope, internal site names under websites/, hardcoded repo paths for
//    gh api, metrics years, lockfile names, and dated snapshots assume this
//    monorepo's state. Skills use placeholder forms (@<scope>/<pkg>,
//    websites/<site>, repos/{owner}/{repo}, {YYYY}, <lockfile>). Skills
//    derive data live.
...
// these rules. fit-*, gemba/gemba-*, and jidoka-* skills are out of scope.
// They document their own published CLIs. They legitimately name
// @forwardimpact packages and tool integrations. This rule scans only kata-*
...
// convention agrees with the frontmatter classifier the runtime loader uses.
// A file with name+description frontmatter is a profile. Its name must NOT
// start with x-. A file without that frontmatter is a reference. Its name
// MUST start with x-. This catches a reference that grew agent frontmatter
// and would load as a subagent. It also catches a profile that took the
...
const PATTERNS = [
  ...
  // `just` is also an adverb. So match only command-shaped forms: backticked,
  ...
  // The sync omits root TRUST.md and the fit-* skills (a separate pack), so
  ...
  {
    pattern: "\\]\\((\\.\\./)+TRUST\\.md",
    reason: "the pack does not ship TRUST.md — use the full GitHub URL",
  },
  {
    pattern: "\\]\\((\\.\\./)+(fit-|gemba[-/])",
    reason:
...
export default {
  ...
  rules: ({ failAll }) => [
    failAll("skill-match", {
      ...
      hint: "published pack content (kata-* skills and agent references) must hold in a repo that installed the pack yesterday (.claude/skills/CLAUDE.md § Generic by design). Narrow the rule in .jidoka/invariants/skill-genericity.rules.mjs only for a legitimate generic usage",
    }),
