...
`scripts/bootstrap.sh` is mandatory. The CI gemba-bootstrap action invokes it by
path with no fallback. A repo without it fails every agent and check workflow at
