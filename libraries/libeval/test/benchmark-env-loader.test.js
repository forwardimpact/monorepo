...
const FAMILY_DIR = "/env-family";
const TASK_DIR = "/env-task";
const AGENT_CWD = "/env-cwd";

/**
 * Build a runtime over an in-memory fs seeded with `files`, an isolated env
 * map, and a captured stderr. `loadEnv` reads, renders, and writes entirely
 * through `runtime.fs`, so the seeded map is the on-disk env it operates on.
 */
...
describe("loadEnv", () => {
  test("loads .env into the env map and renders into agent CWD", async () => {
    const rt = makeRuntime({
      files: { [`${FAMILY_DIR}/.env`]: "TEST_FAM_VAR=from-family\n" },
    });
    const names = await loadEnv([FAMILY_DIR], AGENT_CWD, rt.runtime);
    assert.ok(names.includes("TEST_FAM_VAR"));
    assert.strictEqual(rt.env.TEST_FAM_VAR, "from-family");

    const rendered = await rt.fs.readFile(`${AGENT_CWD}/.env`, "utf8");
    assert.ok(rendered.includes("TEST_FAM_VAR=from-family"));
  });

  test("existing env value wins over file values", async () => {
    const rt = makeRuntime({
      env: { TEST_EXISTING: "from-shell" },
      files: { [`${FAMILY_DIR}/.env`]: "TEST_EXISTING=from-file\n" },
    });
    await loadEnv([FAMILY_DIR], AGENT_CWD, rt.runtime);
  ...
  test("merges family and task dirs, family loaded first", async () => {
    const rt = makeRuntime({
      files: {
        ...
        [`${TASK_DIR}/.env`]: "SHARED=from-task\nTASK_ONLY=2\n",
      },
    });
    const names = await loadEnv([FAMILY_DIR, TASK_DIR], AGENT_CWD, rt.runtime);
  ...
  test("renders .env.local into agent CWD", async () => {
    const rt = makeRuntime({
      files: { [`${TASK_DIR}/.env.local`]: "SECRET=s3cr3t\n" },
    });
    await loadEnv([TASK_DIR], AGENT_CWD, rt.runtime);

    const rendered = await rt.fs.readFile(`${AGENT_CWD}/.env.local`, "utf8");
  ...
  test("renders both .env and .env.local when both exist", async () => {
    const rt = makeRuntime({
      files: {
        [`${TASK_DIR}/.env`]: "A=1\n",
        [`${TASK_DIR}/.env.local`]: "B=2\n",
      },
    });
    await loadEnv([TASK_DIR], AGENT_CWD, rt.runtime);

    const dotenv = await rt.fs.readFile(`${AGENT_CWD}/.env`, "utf8");
    const dotenvLocal = await rt.fs.readFile(`${AGENT_CWD}/.env.local`, "utf8");
  ...
  test("returns empty array when no env files exist", async () => {
    ...
    await assert.rejects(rt.fs.readFile(`${AGENT_CWD}/.env`, "utf8"), {
      code: "ENOENT",
    });
  });
  ...
  test("CI secret overrides file value in rendered output", async () => {
    const rt = makeRuntime({
      env: { TASK_SECRET: "from-ci" },
      files: { [`${TASK_DIR}/.env.local`]: "TASK_SECRET=placeholder\n" },
    });
    await loadEnv([TASK_DIR], AGENT_CWD, rt.runtime);
  ...
  test("emits warning to stderr for empty-value vars", async () => {
    const rt = makeRuntime({
      files: { [`${TASK_DIR}/.env`]: "MISSING_VAR=\n" },
    });
    await loadEnv([TASK_DIR], AGENT_CWD, rt.runtime);
