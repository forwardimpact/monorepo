...
export class BenchmarkRunner {
  ...
  async #runOne(family, wm, task, runIndex, skillSetHash) {
    ...
    try {
      ...
      const { costUsd, turns, submission } = this._runAgentHook
  ...
  async #runAgent(task, workdir) {
    ...
    try {
      await runner.run(instructions);
    } catch {
      // Swallow — scoring/judge still need to run against whatever state the
      // workdir is in (spec criterion 1 demands a record on agent failure).
    } finally {
      await new Promise((r) => agentTraceStream.end(r));
    }
    return await readAgentSummary(workdir.agentTracePath);
  }
