 ...
 * closure backwards from there and memoizes each node per run. Nodes are
 * pure functions of their declared dependencies. Side-effects (writes,
 ...
 *
 * Verb terminal-closure walks (the payoff of the cost shift in Phase D):
 *   check    → parse, entities, prose-keys, cache-lookup
 *   validate → ...above + datasets, fhir-cross-ref, skeleton, enriched,
 *              validate (the skeleton → fhir-cross-ref dependency pulls
 *              datasets)
...
/** Names of the nodes the DAG knows about. The inspect verb takes one. */
export const STAGES = [
  "parse",
  "entities",
  "prose-keys",
...
/**
 * Walk the DAG backwards from `terminal`. Execute each node once and
 * memoize its output. Returns the terminal node's output, the per-run
...
export class Pipeline {
  ...
  async run(options) {
    ...
    // Persist any prose that enrichment and pathway generate after the
    // cache-lookup flush. cache-lookup flushes prose-key entries. But
    // generateStructured() calls inside `enriched` and `pathway` write
    // additional entries. The process would otherwise lose them on exit.
