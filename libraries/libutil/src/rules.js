// Generic rule-execution engine. It pairs with `libutil/findings.js` for
// output.
//
...
// - `when(subject, ctx)` is an optional predicate. A falsy result skips the
//   rule.
...
// - `hint` is an optional action prompt. The text emitter renders it. It is
//   either a static string or a `(subject, item, ctx) => string` that resolves
//   once per finding. A function hint lets a rule emit a fully resolved
//   command from the subject (e.g. a `rotate --agent <prefix>` that names the
//   flagged file's agent).
//
// The engine passes `ctx` unchanged to every rule. Cross-subject state (e.g.,
// a duplicate-detection map) lives on `ctx`. The rule mutates it during the
