...
describe("validateCSV", () => {
  ...
  test("rejects wrong header with a column diff", () => {
    ...
    // spec 1910 added `host_run` to COLUMNS, so the column diff now lists both
  ...
  test("accepts the 8-column header with host_run (spec 1910)", () => {
    const csv = [
  ...
  test("accepts the legacy 7-column header (spec 1910 criterion 6)", () => {
    const csv = [
      "date,metric,value,unit,run,note,event_type",
      "2026-01-01,bugs,3,count,https://example.com,,kata-shift",
    ].join("\n");
