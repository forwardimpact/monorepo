...
describe("checkInvariants", () => {
  ...
  test("rejects when the rules dir is missing", async () => {
    ...
    await assert.rejects(
      checkInvariants({ root: "/nowhere", runtime }),
...
describe("findInvariantsRoot", () => {
  test("prefers the rules dir over the nearest package.json", () => {
    const runtime = createTestRuntime({
      proc: { cwd: () => "/repo/libraries/libfoo/src" },
      finder: {
        findUpward: (start, rel) =>
          rel === ".coaligned/invariants"
            ? "/repo/.coaligned/invariants"
            : null,
        findProjectRoot: () => "/repo/libraries/libfoo",
      },
    });
    assert.equal(findInvariantsRoot(runtime), "/repo");
  });
  ...
  test("falls back to the nearest project root", () => {
    ...
    assert.equal(findInvariantsRoot(runtime), "/somewhere/else");
  });
});
