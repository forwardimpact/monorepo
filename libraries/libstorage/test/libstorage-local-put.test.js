...
describe("LocalStorage put atomicity — spy fs", () => {
  ...
  test("writeFile failure unlinks tmp and rejects with the original error", async () => {
  ...
  test("rename failure unlinks tmp and rejects with the original error", async () => {
  ...
  test("put swallows the cleanup error and propagates the original error", async () => {
  ...
  test("findByExtension skips tmp survivors that share the canonical extension", async () => {
    mockFs.readdir = spy(() =>
      Promise.resolve([
        {
          name: "real.json",
...
describe("LocalStorage put atomicity — real fs", () => {
  ...
  test("orphan tmp survivor is invisible to the list methods", async () => {
    await storage.put("real.json", '{"k":1}');
    // Simulate a tmp survivor from a put that a process kill interrupted.
    await fsp.writeFile(
    ...
    // The orphan file remains on disk. The operator owns its removal. The
    // file never surfaces through the API.
  ...
  test("failed put leaves target byte-equal to prior content and unlinks tmp", async () => {
    ...
    // Wrap fs so the second put's rename throws after writeFile created a
    // real tmp on disk. The wrapper does NOT unlink, so #unlinkBestEffort
    // owns the cleanup. The failure mode of a kernel rename stays honest.
    ...
    // The target survives at prior content. #unlinkBestEffort cleared the tmp.
    assert.strictEqual(
  ...
  test("concurrent put on same key: both resolve, target ends at one input verbatim, and no tmp leaks", async () => {
    ...
    // ends byte-equal to one of the inputs. It is not a serial concat or a
    // partial. No tmp sibling leaks.
