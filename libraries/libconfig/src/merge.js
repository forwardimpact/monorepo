/**
 * Pure namespace-ownership classifier. `bootstrapProject` uses it.
 *
 * `mergeConfigFragment` enforces ownership at the **top-level key**. It
 * surfaces diagnostics at the **leaf path** that disagrees, so the caller
 * can pinpoint the conflict. `mergeEnvEntries` applies the same three rows
 * at bare-key granularity.
 */
...
/**
 * Recursively merge `fragment` into `existing`. Record each per-leaf
 * conflict at its dotted path. When both sides are plain objects with
 * disjoint sub-keys (no leaf disagrees), the result is the deep-merge of
 * the two subtrees. This applies the cross-namespace-always-succeeds rule
 * one level deeper than the top-level. When a leaf disagrees (or shapes
 * mismatch), this function records the conflict at that leaf path. It also
 * keeps the existing value in the partial result. The orchestrator discards
 * that partial result when conflicts is non-empty.
 */
 ...
 * @param {string[]} [params.overwrites]  Top-level keys the caller approved
...
/**
 * Classify `.env` entries at bare-key granularity. This function compares
 * values byte-for-byte after `KEY=`.
 * @param {object} params
