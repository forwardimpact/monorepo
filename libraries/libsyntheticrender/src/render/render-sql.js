 ...
 * @param {object} outputConfig - `{ prefix, entities, include_embeddings }`
...
export function renderSql(clinicalEntities, outputConfig) {
  ...
  for (const spec of includedSpecs) {
    ...
    files.set(
      `${prefix}_${next()}_${spec.table}.sql`,
  ...
  files.set(`${prefix}_${next()}_rls.sql`, renderRls(allTables));

  if (outputConfig.include_embeddings) {
    files.set(
      `${prefix}_${next()}_condition_embeddings.sql`,
